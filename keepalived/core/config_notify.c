...
static void child_reloaded_thread(__attribute__((unused)) thread_ref_t thread) {
...
void save_config(bool post, const char *process, void (*func)(FILE *)) {
  static unsigned reload_num = 0;
  FILE *file;
  char buf[128];

  if (!config_save_dir) return;

  if (!post) reload_num++;

  sprintf(
      buf, "%s/keepalived_%s.%d.%u.%s", config_save_dir, process, getpid(), reload_num,
      post ? "post" : "pre");
...
#ifdef THREAD_DUMP
void register_config_notify_addresses(void) {
