...
static void use_polling_handler(const vector_t *strvec) {
  if (!strvec) return;
...
static void save_process_name(char const **dest, const char *src) {
  size_t len;
  ...
  if (*dest) FREE_CONST_PTR(*dest);

  if ((len = strlen(src)) >= TASK_COMM_LEN) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Process name %s more than %d characters, truncating", src,
        TASK_COMM_LEN - 1);
    len = TASK_COMM_LEN - 1;
  }
...
static void process_names_handler(__attribute__((unused)) const vector_t *strvec) {
#ifdef _WITH_VRRP_
...
static void process_name_handler(const vector_t *strvec) {
  save_process_name(&global_data->process_name, strvec_slot(strvec, 1));
}
#ifdef _WITH_VRRP_
static void vrrp_process_name_handler(const vector_t *strvec) {
  save_process_name(&global_data->vrrp_process_name, strvec_slot(strvec, 1));
}
#endif
#ifdef _WITH_LVS_
static void checker_process_name_handler(const vector_t *strvec) {
  save_process_name(&global_data->lvs_process_name, strvec_slot(strvec, 1));
}
static void lvs_process_name_handler(const vector_t *strvec) {
...
#endif
#ifdef _WITH_BFD_
static void bfd_process_name_handler(const vector_t *strvec) {
  save_process_name(&global_data->bfd_process_name, strvec_slot(strvec, 1));
}
#endif
static void routerid_handler(const vector_t *strvec) {
...
static void emailfrom_handler(const vector_t *strvec) {
  if (vector_size(strvec) < 2) {
...
static void smtpto_handler(const vector_t *strvec) {
  unsigned timeout;
  ...
  if (!read_unsigned_strvec(strvec, 1, &timeout, 0, UINT_MAX / TIMER_HZ, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "smtp_connect_timeout '%s' must be in [0, %u] - ignoring",
        strvec_slot(strvec, 1), UINT_MAX / TIMER_HZ);
    return;
  }

  if (timeout == 0) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "smtp_conect_timeout must be greater than 0, setting to 1");
    timeout = 1;
  }
...
#ifdef _WITH_VRRP_
static void dynamic_interfaces_handler(const vector_t *strvec) {
  const char *str;
...
static void no_email_faults_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->no_email_faults = true;
}
#endif
static void smtpserver_handler(const vector_t *strvec) {
  bool ret = true;
  const char *port_str = SMTP_PORT_STR;

  /* Has a port number been specified? */
  if (vector_size(strvec) >= 3) port_str = strvec_slot(strvec, 2);
  ...
  if (ret) domain_stosockaddr(strvec_slot(strvec, 1), port_str, &global_data->smtp_server);

  if (global_data->smtp_server.ss_family == AF_UNSPEC)
    report_config_error(
        CONFIG_GENERAL_ERROR, "Invalid smtp server %s %s", strvec_slot(strvec, 1), port_str);
}
static void smtphelo_handler(const vector_t *strvec) {
  if (vector_size(strvec) < 2) return;

  global_data->smtp_helo_name = STRDUP(strvec_slot(strvec, 1));
}
static void email_handler(const vector_t *strvec) {
...
static void smtp_alert_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value '%s' for global smtp_alert specified",
...
static void startup_shutdown_script(
    const vector_t *strvec, notify_script_t **script, bool startup) {
  const char *type = startup ? "startup" : "shutdown";

#ifndef _ONE_PROCESS_DEBUG_
  if (prog_type != PROG_TYPE_PARENT) return;
#endif
  ...
  if (startup) {
    if (!global_data->startup_script_timeout) global_data->startup_script_timeout = 10;
  } else {
    if (!global_data->shutdown_script_timeout) global_data->shutdown_script_timeout = 10;
  }
}

static void startup_shutdown_script_timeout_handler(const vector_t *strvec, bool startup) {
  const char *type = startup ? "startup" : "shutdown";
  unsigned delay;
  ...
  if (!read_unsigned_strvec(strvec, 1, &delay, 1, 1000, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "%s_script_timeout '%s' must be in [1, 1000] - ignoring", type,
        strvec_slot(strvec, 1));
    return;
  }
...
static void startup_script_handler(const vector_t *strvec) {
  /* Only applicable for the parent process */
  startup_shutdown_script(strvec, &global_data->startup_script, true);
}

static void startup_script_timeout_handler(const vector_t *strvec) {
  /* Only applicable for the parent process */
  startup_shutdown_script_timeout_handler(strvec, true);
}

static void shutdown_script_handler(const vector_t *strvec) {
  startup_shutdown_script(strvec, &global_data->shutdown_script, false);
}

static void shutdown_script_timeout_handler(const vector_t *strvec) {
  startup_shutdown_script_timeout_handler(strvec, false);
}

static void max_auto_priority_handler(const vector_t *strvec) {
  int priority;
  ...
  if (!read_int_strvec(strvec, 1, &priority, -1, max_priority, true)) {
    report_config_error(
        ...
        "max_auto_priority '%s' must be in [0, %d] (or -1 to disable) - ignoring",
        strvec_slot(strvec, 1), max_priority);
    return;
  }

  global_data->max_auto_priority = priority;
}
static void min_auto_priority_delay_handler(const vector_t *strvec) {
  int delay;
  ...
  if (!read_int_strvec(strvec, 1, &delay, 1, 10000000, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "min_auto_priority_delay '%s' must be in [1, 10000000] - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
...
#ifdef _WITH_VRRP_
static void smtp_alert_vrrp_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value '%s' for global smtp_alert_vrrp specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
#endif
#ifdef _WITH_LVS_
static void smtp_alert_checker_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value '%s' for global smtp_alert_checker specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void checker_log_all_failures_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value for checker_log_all_failures specified");
      return;
    }
  }
...
#ifdef _WITH_VRRP_
static void default_interface_handler(const vector_t *strvec) {
  if (vector_size(strvec) < 2) {
...
static void disable_local_igmp_handler(__attribute__((unused)) const vector_t *strvec) {
  if (access(igmp_link_local_mcast_reports, W_OK)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "kernel does not support %s", igmp_link_local_mcast_reports);
    return;
  }

  global_data->disable_local_igmp = true;
}
static void v3_checksum_as_v2(__attribute__((unused)) const vector_t *strvec) {
  global_data->v3_checksum_as_v2 = true;
}
#endif
#ifdef _WITH_LVS_
static void lvs_timeouts(const vector_t *strvec) {
  unsigned val;
  size_t i;
  ...
  for (i = 1; i < vector_size(strvec); i++) {
    if (!strcmp(strvec_slot(strvec, i), "tcp")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_timeout tcp - ignoring");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 0, LVS_MAX_TIMEOUT, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_timeout tcp (%s) - ignoring",
            strvec_slot(strvec, i + 1));
      else
    ...
    if (!strcmp(strvec_slot(strvec, i), "tcpfin")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_timeout tcpfin - ignoring");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 0, LVS_MAX_TIMEOUT, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_timeout tcpfin (%s) - ignoring",
            strvec_slot(strvec, i + 1));
      else
    ...
    if (!strcmp(strvec_slot(strvec, i), "udp")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_timeout udp - ignoring");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 0, LVS_MAX_TIMEOUT, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_timeout udp (%s) - ignoring",
            strvec_slot(strvec, i + 1));
      else
    ...
    report_config_error(
        CONFIG_GENERAL_ERROR, "Unknown option %s specified for lvs_timeouts",
        strvec_slot(strvec, i));
  }
}
#if defined _WITH_LVS_ && defined _WITH_VRRP_
static void lvs_syncd_handler(const vector_t *strvec) {
  unsigned val;
  size_t i;

  if (global_data->lvs_syncd.ifname) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "lvs_sync_daemon has already been specified as %s %s - ignoring",
        global_data->lvs_syncd.ifname,
        global_data->lvs_syncd.vrrp_name ? global_data->lvs_syncd.vrrp_name : "");
    return;
  }
  ...
  if (strlen(strvec_slot(strvec, 1)) >= IP_VS_IFNAME_MAXLEN) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "lvs_sync_daemon interface name '%s' too long - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  for (i = 2; i < vector_size(strvec); i++) {
    ...
    if (!strcmp(strvec_slot(strvec, i), "inst")) {
      if (global_data->lvs_syncd.vrrp_name)
        report_config_error(
            CONFIG_GENERAL_ERROR,
            "lvs_sync_daemon vrrp instance has already been specified as %s - ignoring",
    ...
    if (!strcmp(strvec_slot(strvec, i), "id")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_sync_daemon id, defaulting to vrid");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 0, 255, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid syncid (%s) - defaulting to vrid",
            strvec_slot(strvec, i + 1));
      else
    ...
    if (!strcmp(strvec_slot(strvec, i), "maxlen")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_sync_daemon maxlen - ignoring");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 1, 65535 - 20 - 8, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_sync_daemon maxlen (%s) - ignoring",
            strvec_slot(strvec, i + 1));
      else
    ...
    if (!strcmp(strvec_slot(strvec, i), "port")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_sync_daemon port - ignoring");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 1, 65535, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_sync_daemon port (%s) - ignoring",
            strvec_slot(strvec, i + 1));
      else
    ...
    if (!strcmp(strvec_slot(strvec, i), "ttl")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_sync_daemon ttl - ignoring");
        continue;
      }
      if (!read_unsigned_strvec(strvec, i + 1, &val, 1, 255, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_sync_daemon ttl (%s) - ignoring",
            strvec_slot(strvec, i + 1));
      else
    ...
    if (!strcmp(strvec_slot(strvec, i), "group")) {
      if (i == vector_size(strvec) - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "No value specified for lvs_sync_daemon group - ignoring");
        continue;
      }

      if (inet_stosockaddr(strvec_slot(strvec, i + 1), NULL, &global_data->lvs_syncd.mcast_group))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid lvs_sync_daemon group (%s) - ignoring",
            strvec_slot(strvec, i + 1));

      if ((global_data->lvs_syncd.mcast_group.ss_family == AF_INET &&
           !IN_MULTICAST(htonl(PTR_CAST(struct sockaddr_in, &global_data->lvs_syncd.mcast_group)
                                   ->sin_addr.s_addr))) ||
          (global_data->lvs_syncd.mcast_group.ss_family == AF_INET6 &&
           !IN6_IS_ADDR_MULTICAST(
               &PTR_CAST(struct sockaddr_in6, &global_data->lvs_syncd.mcast_group)->sin6_addr))) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "lvs_sync_daemon group address %s is not multicast - ignoring",
            strvec_slot(strvec, i + 1));
    ...
    /* The following are for backward compatibility when lvs_sync_daemon IF VRRP_INSTANCE [SYNC_ID]
     * could be specified */
    if (i == 2) {
    ...
    if (i == 3) {
      if (!read_unsigned_strvec(strvec, 3, &val, 0, 255, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid syncid (%s) - defaulting to vrid",
            strvec_slot(strvec, 3));
      else {
        report_config_error(
            CONFIG_GENERAL_ERROR, "Please use keyword \"id\" before lvs_sync_daemon SYNCID");
        global_data->lvs_syncd.syncid = val;
      }
    ...
    report_config_error(
        CONFIG_GENERAL_ERROR, "Unknown option %s specified for lvs_sync_daemon",
        strvec_slot(strvec, i));
  }
}
#endif
static void lvs_flush_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->lvs_flush = true;
}

static void lvs_flush_on_stop_handler(const vector_t *strvec) {
  if (vector_size(strvec) == 1)
    ...
    report_config_error(
        CONFIG_GENERAL_ERROR, "Unknown lvs_flush_on_stop type %s", strvec_slot(strvec, 1));
}
#endif

static int get_realtime_priority(const vector_t *strvec, const char *process) {
  int min_priority;
  int max_priority;
  int priority;
  ...
  if (vector_size(strvec) < 2) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "No %s process real-time priority specified", process);
    return -1;
  }
  ...
  if (!read_int_strvec(strvec, 1, &priority, INT_MIN, INT_MAX, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "%s process real-time priority '%s' invalid", process,
        strvec_slot(strvec, 1));
    return -1;
  }
  ...
  if (priority < min_priority) {
    report_config_error(
        CONFIG_GENERAL_ERROR,
        "%s process real-time priority %d less than minimum %d - setting to minimum", process,
        priority, min_priority);
    priority = min_priority;
  } else if (priority > max_priority) {
    report_config_error(
        CONFIG_GENERAL_ERROR,
        "%s process real-time priority %d greater than maximum %d - setting to maximum", process,
...
static int get_cpu_affinity(const vector_t *strvec, cpu_set_t *set, const char *process) {
  int cpu_id, num_cpus;
  unsigned i;
  ...
  for (i = 1; i < vector_size(strvec); i++) {
    if (!read_int_strvec(strvec, i, &cpu_id, 0, num_cpus - 1, true)) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid cpu_id:%d specified for %s process", cpu_id, process);
...
static rlim_t get_rt_rlimit(const vector_t *strvec, const char *process) {
  unsigned limit;
  rlim_t rlim;
  size_t keyword_len;
  ...
  if (strvec_slot(strvec, 0)[keyword_len - 5] == 'r')
    log_message(
        LOG_INFO, "Keyword '%s' is deprecated - please use '%.*srttime'", strvec_slot(strvec, 0),
        (int)keyword_len - 5, strvec_slot(strvec, 0));

  if (!read_unsigned_strvec(strvec, 1, &limit, 1, UINT32_MAX, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Invalid %s real-time limit - %s", process, strvec_slot(strvec, 1));
    return 0;
  }
...
static int8_t get_priority(const vector_t *strvec, const char *process) {
  int priority;
...
#ifdef _WITH_VRRP_
static void vrrp_mcast_group4_handler(const vector_t *strvec) {
  sockaddr_t mcast = {.ss_family = AF_UNSPEC};

  if (inet_stosockaddr(strvec_slot(strvec, 1), NULL, &mcast)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Can't parse vrrp_mcast_group4 [%s]. Skipping",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (mcast.ss_family != AF_INET) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_mcast_group4 [%s] is not IPv4. Skipping",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (!IN_MULTICAST(htonl(PTR_CAST(struct sockaddr_in, &mcast)->sin_addr.s_addr))) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_mcast_group4 [%s] is not multicast. Skipping",
        strvec_slot(strvec, 1));
    return;
  }
...
static void vrrp_mcast_group6_handler(const vector_t *strvec) {
  sockaddr_t mcast = {.ss_family = AF_UNSPEC};

  if (inet_stosockaddr(strvec_slot(strvec, 1), NULL, &mcast)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Can't parse vrrp_mcast_group6 [%s]. Skipping",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (mcast.ss_family != AF_INET6) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_mcast_group6 [%s] is not IPv6. Skipping",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (!IN6_IS_ADDR_MULTICAST(&PTR_CAST(struct sockaddr_in6, &mcast)->sin6_addr)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_mcast_group6 [%s] is not multicast. Skipping",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (!IN6_IS_ADDR_MC_LINKLOCAL(&PTR_CAST(struct sockaddr_in6, &mcast)->sin6_addr))
    report_config_error(
        CONFIG_WARNING, "vrrp_mcast_group6 [%s] should be link-local multicast.",
        strvec_slot(strvec, 1));

  global_data->vrrp_mcast_group6 = *PTR_CAST(struct sockaddr_in6, &mcast);
}
static void vrrp_garp_delay_handler(const vector_t *strvec) {
  unsigned timeout;

  if (!read_unsigned_strvec(strvec, 1, &timeout, 0, UINT_MAX / TIMER_HZ, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_garp_master_delay '%s' invalid - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
...
static void vrrp_garp_rep_handler(const vector_t *strvec) {
  unsigned repeats;
  ...
  if (!read_unsigned_strvec(strvec, 1, &repeats, 0, UINT_MAX, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_garp_master_repeat '%s' invalid - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (repeats == 0) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_garp_master_repeat must be greater than 0, setting to 1");
    repeats = 1;
  }

  global_data->vrrp_garp_rep = repeats;
}
static void vrrp_garp_refresh_handler(const vector_t *strvec) {
  unsigned refresh;

  if (!read_unsigned_strvec(strvec, 1, &refresh, 0, UINT_MAX, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Invalid vrrp_garp_master_refresh '%s' - ignoring",
        strvec_slot(strvec, 1));
...
static void vrrp_garp_refresh_rep_handler(const vector_t *strvec) {
  unsigned repeats;
  ...
  if (!read_unsigned_strvec(strvec, 1, &repeats, 0, UINT_MAX, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_garp_master_refresh_repeat '%s' invalid - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
  ...
  if (repeats == 0) {
    report_config_error(
        CONFIG_GENERAL_ERROR,
        "vrrp_garp_master_refresh_repeat must be greater than 0, setting to 1");
    repeats = 1;
  }

  global_data->vrrp_garp_refresh_rep = repeats;
}
static void vrrp_garp_lower_prio_delay_handler(const vector_t *strvec) {
  unsigned delay;

  if (!read_unsigned_strvec(strvec, 1, &delay, 0, UINT_MAX / TIMER_HZ, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_garp_lower_prio_delay '%s' invalid - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
...
static void vrrp_garp_lower_prio_rep_handler(const vector_t *strvec) {
  unsigned garp_lower_prio_rep;

  if (!read_unsigned_strvec(strvec, 1, &garp_lower_prio_rep, 0, INT_MAX, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Invalid vrrp_garp_lower_prio_repeat '%s'", strvec_slot(strvec, 1));
    return;
  }

  global_data->vrrp_garp_lower_prio_rep = garp_lower_prio_rep;
}
static void vrrp_down_timer_adverts_handler(const vector_t *strvec) {
  unsigned down_timer_adverts;

  if (!read_unsigned_strvec(strvec, 1, &down_timer_adverts, 1, 100, true)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Invalid vrrp_down_timer_adverts [1:100] '%s'",
        strvec_slot(strvec, 1));
    return;
  }
...
static void vrrp_garp_interval_handler(const vector_t *strvec) {
  unsigned interval;

  if (!read_decimal_unsigned_strvec(strvec, 1, &interval, 1, UINT_MAX, TIMER_HZ_DIGITS, true))
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_garp_interval '%s' is invalid", strvec_slot(strvec, 1));
  else
    global_data->vrrp_garp_interval = interval;

  if (global_data->vrrp_garp_interval >= 1 * TIMER_HZ)
    log_message(
        LOG_INFO, "The vrrp_garp_interval is very large - %s seconds", strvec_slot(strvec, 1));
}
static void vrrp_gna_interval_handler(const vector_t *strvec) {
  unsigned interval;

  if (!read_decimal_unsigned_strvec(strvec, 1, &interval, 1, UINT_MAX, TIMER_HZ_DIGITS, true))
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_gna_interval '%s' is invalid", strvec_slot(strvec, 1));
  else
    global_data->vrrp_gna_interval = interval;

  if (global_data->vrrp_gna_interval >= 1 * TIMER_HZ)
    log_message(
        LOG_INFO, "The vrrp_gna_interval is very large - %s seconds", strvec_slot(strvec, 1));
}
static void vrrp_min_garp_handler(const vector_t *strvec) {
  int res = false;
  ...
  if (global_data->vrrp_garp_rep == VRRP_GARP_REP) global_data->vrrp_garp_rep = 1;
  ...
  if (global_data->vrrp_garp_delay == VRRP_GARP_DELAY) global_data->vrrp_garp_delay = 0;
}
#ifdef _HAVE_VRRP_VMAC_
static void vrrp_vmac_garp_extra_if_handler(const vector_t *strvec) {
  unsigned delay = 0;
  unsigned index;
  ...
  if (!strcmp(cmd_name, "vrrp_vmac_garp_intvl")) {
    ...
    report_config_error(
        CONFIG_DEPRECATED,
        "Keyword \"vrrp_vmac_garp_intvl\" is deprecated - please use \"vrrp_garp_extra_if\"");
  }
  ...
  for (index = 1; index < vector_size(strvec); index++) {
    if (!strcmp(strvec_slot(strvec, index), "all"))
      ...
      report_config_error(
          CONFIG_GENERAL_ERROR, "%s '%s' invalid - ignoring", cmd_name, strvec_slot(strvec, index));
      return;
    }
  }
...
#endif
static void vrrp_lower_prio_no_advert_handler(const vector_t *strvec) {
  int res;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0)
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value for vrrp_lower_prio_no_advert specified");
    else
      global_data->vrrp_lower_prio_no_advert = res;
  } else
    global_data->vrrp_lower_prio_no_advert = true;
}
static void vrrp_higher_prio_send_advert_handler(const vector_t *strvec) {
  int res;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0)
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value for vrrp_higher_prio_send_advert specified");
    else
...
static void vrrp_iptables_handler(const vector_t *strvec) {
  ...
  if (vector_size(strvec) >= 2) {
    if (strlen(strvec_slot(strvec, 1)) >= XT_EXTENSION_MAXNAMELEN - 1) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "VRRP Error : iptables in chain name too long - ignored");
      return;
    }
    ...
    if (vector_size(strvec) >= 3) {
      if (strlen(strvec_slot(strvec, 2)) >= XT_EXTENSION_MAXNAMELEN - 1) {
        report_config_error(
            CONFIG_GENERAL_ERROR, "VRRP Error : iptables out chain name too long - ignored");
        return;
      }
...
static void vrrp_ipsets_handler(const vector_t *strvec) {
  size_t len;
  ...
  if (vector_size(strvec) >= 3) {
    if (strlen(strvec_slot(strvec, 2)) >= IPSET_MAXNAMELEN - 1) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "VRRP Error : ipset IPv6 address name too long - ignored");
      return;
    }
  ...
  if (vector_size(strvec) >= 4) {
    if (strlen(strvec_slot(strvec, 3)) >= IPSET_MAXNAMELEN - 1) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "VRRP Error : ipset IPv6 address_iface name too long - ignored");
      return;
    }
    ...
    len = strlen(set_name);
    if (set_name[len - 1] == '6') set_name[--len] = '\0';
...
static void vrrp_iptables_handler(__attribute__((unused)) const vector_t *strvec) {
  report_config_error(
      CONFIG_GENERAL_ERROR,
      "iptables not supported, using nftables instead. Please replace 'vrrp_iptables and "
      "'vrrp_ipsets' with 'nftables' config option");
...
#ifdef _WITH_NFTABLES_
#ifdef _WITH_VRRP_
static void vrrp_nftables_handler(__attribute__((unused)) const vector_t *strvec) {
  const char *name;
...
static void vrrp_nftables_priority_handler(const vector_t *strvec) {
  int priority;
  ...
  if (read_int_strvec(strvec, 1, &priority, INT32_MIN, INT32_MAX, false))
    ...
    report_config_error(
        CONFIG_INVALID_NUMBER, "invalid nftables chain priority '%s'", strvec_slot(strvec, 1));
}
static void vrrp_nftables_ifindex_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->vrrp_nf_ifindex = true;
}
#endif

#ifdef _WITH_LVS_
static void ipvs_nftables_handler(__attribute__((unused)) const vector_t *strvec) {
  const char *name;
...
static void ipvs_nftables_priority_handler(const vector_t *strvec) {
  int priority;
  ...
  if (read_int_strvec(strvec, 1, &priority, INT32_MIN, INT32_MAX, false))
    ...
    report_config_error(
        CONFIG_INVALID_NUMBER, "invalid ipvs nftables chain priority '%s'", strvec_slot(strvec, 1));
}
static void ipvs_nftables_start_fwmark_handler(const vector_t *strvec) {
  unsigned fwmark;
  ...
  if (read_unsigned_strvec(strvec, 1, &fwmark, 1, UINT32_MAX, false))
    ...
    report_config_error(
        CONFIG_INVALID_NUMBER, "invalid ipvs nftables start_fwmark priority '%s'",
        strvec_slot(strvec, 1));
}
#endif

static void nftables_counters_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->nf_counters = true;
}
#endif
static void vrrp_version_handler(const vector_t *strvec) {
  int version;
...
static void vrrp_check_unicast_src_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->vrrp_check_unicast_src = 1;
}
static void vrrp_check_adv_addr_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->vrrp_skip_check_adv_addr = 1;
}
static void vrrp_strict_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->vrrp_strict = 1;
}
static void vrrp_prio_handler(const vector_t *strvec) {
  global_data->vrrp_process_priority = get_priority(strvec, "vrrp");
}
static void vrrp_no_swap_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->vrrp_no_swap = true;
}

static void vrrp_rt_priority_handler(const vector_t *strvec) {
  int priority = get_realtime_priority(strvec, "vrrp");

  if (priority >= 0) global_data->vrrp_realtime_priority = priority;
}
static void vrrp_cpu_affinity_handler(const vector_t *strvec) {
  get_cpu_affinity(strvec, &global_data->vrrp_cpu_mask, "vrrp");
}
static void vrrp_rt_rlimit_handler(const vector_t *strvec) {
  global_data->vrrp_rlimit_rt = get_rt_rlimit(strvec, "vrrp");
}
#endif

static void notify_fifo(const vector_t *strvec, const char *type, notify_fifo_t *fifo) {
  if (vector_size(strvec) < 2) {
  ...
  if (fifo->name) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "%snotify_fifo already specified - ignoring %s", type,
        strvec_slot(strvec, 1));
    return;
  }
...
static void notify_fifo_script(const vector_t *strvec, const char *type, notify_fifo_t *fifo) {
  char *id_str;
  ...
  if (fifo->script) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "%snotify_fifo_script already specified - ignoring %s", type,
        strvec_slot(strvec, 1));
    return;
  }
...
static void global_notify_fifo(const vector_t *strvec) {
  notify_fifo(strvec, "", &global_data->notify_fifo);
}
static void global_notify_fifo_script(const vector_t *strvec) {
  notify_fifo_script(strvec, "", &global_data->notify_fifo);
}
#ifdef _WITH_VRRP_
static void vrrp_notify_fifo(const vector_t *strvec) {
  notify_fifo(strvec, "vrrp_", &global_data->vrrp_notify_fifo);
}
static void vrrp_notify_fifo_script(const vector_t *strvec) {
  notify_fifo_script(strvec, "vrrp_", &global_data->vrrp_notify_fifo);
}
static void vrrp_notify_priority_changes(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR,
          "Invalid value '%s' for global vrrp_notify_priority_changes specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void fifo_write_vrrp_states_on_reload(__attribute__((unused)) const vector_t *strvec) {
  global_data->fifo_write_vrrp_states_on_reload = true;
}
#endif
#ifdef _WITH_LVS_
static void lvs_notify_fifo(const vector_t *strvec) {
  notify_fifo(strvec, "lvs_", &global_data->lvs_notify_fifo);
}
static void lvs_notify_fifo_script(const vector_t *strvec) {
  notify_fifo_script(strvec, "lvs_", &global_data->lvs_notify_fifo);
}
#endif
#ifdef _WITH_LVS_
static void checker_prio_handler(const vector_t *strvec) {
  global_data->checker_process_priority = get_priority(strvec, "checker");
}
static void checker_no_swap_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->checker_no_swap = true;
}

static void checker_rt_priority_handler(const vector_t *strvec) {
  int priority = get_realtime_priority(strvec, "checker");

  if (priority >= 0) global_data->checker_realtime_priority = priority;
}
static void checker_cpu_affinity_handler(const vector_t *strvec) {
  get_cpu_affinity(strvec, &global_data->checker_cpu_mask, "checker");
}
static void checker_rt_rlimit_handler(const vector_t *strvec) {
  global_data->checker_rlimit_rt = get_rt_rlimit(strvec, "checker");
}
#endif

#ifdef _WITH_BFD_
static void bfd_prio_handler(const vector_t *strvec) {
  global_data->bfd_process_priority = get_priority(strvec, "bfd");
}
static void bfd_no_swap_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->bfd_no_swap = true;
}

static void bfd_rt_priority_handler(const vector_t *strvec) {
  int priority = get_realtime_priority(strvec, "bfd");

  if (priority >= 0) global_data->bfd_realtime_priority = priority;
}
static void bfd_cpu_affinity_handler(const vector_t *strvec) {
  get_cpu_affinity(strvec, &global_data->bfd_cpu_mask, "bfd");
}
static void bfd_rt_rlimit_handler(const vector_t *strvec) {
  global_data->bfd_rlimit_rt = get_rt_rlimit(strvec, "bfd");
}
#endif

#ifdef _WITH_SNMP_
static void snmp_socket_handler(const vector_t *strvec) {
  if (vector_size(strvec) > 2) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Too many parameters specified for snmp_socket - ignoring");
    return;
  }
  ...
  if (global_data->snmp_socket) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "SNMP socket already set to %s - ignoring", global_data->snmp_socket);
    return;
  }

  global_data->snmp_socket = STRDUP(strvec_slot(strvec, 1));
}
static void trap_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->enable_traps = true;
}
#ifdef _WITH_SNMP_VRRP_
static void snmp_vrrp_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->enable_snmp_vrrp = true;
}
#endif
#ifdef _WITH_SNMP_RFC_
static void snmp_rfc_handler(__attribute__((unused)) const vector_t *strvec) {
...
#endif
#ifdef _WITH_SNMP_RFCV2_
static void snmp_rfcv2_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->enable_snmp_rfcv2 = true;
}
#endif
#ifdef _WITH_SNMP_RFCV3_
static void snmp_rfcv3_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->enable_snmp_rfcv3 = true;
}
#endif
#ifdef _WITH_SNMP_CHECKER_
static void snmp_checker_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->enable_snmp_checker = true;
}
#endif
#endif

static void net_namespace_handler(const vector_t *strvec) {
  if (!strvec) return;
  ...
  if (!global_data->network_namespace) {
    ...
    report_config_error(
        CONFIG_GENERAL_ERROR, "Duplicate net_namespace definition %s - ignoring",
        strvec_slot(strvec, 1));
}

static void net_namespace_ipvs_handler(const vector_t *strvec) {
  if (!strvec) return;

  if (global_data->network_namespace_ipvs) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "Duplicate net_namespace_ipvs definition %s - ignoring",
        strvec_slot(strvec, 1));
    return;
  }
...
static void namespace_ipsets_handler(const vector_t *strvec) {
  if (!strvec) return;
...
#ifdef _WITH_DBUS_
static void enable_dbus_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->enable_dbus = true;
}

static void dbus_service_name_handler(const vector_t *strvec) {
  if (vector_size(strvec) < 2) {
...
static void instance_handler(const vector_t *strvec) {
  if (!strvec) return;
  ...
  if (!reload) {
    if (!global_data->instance_name) {
      ...
      report_config_error(
          CONFIG_GENERAL_ERROR, "Duplicate instance definition %s - ignoring",
          strvec_slot(strvec, 1));
  }
}

static void use_pid_dir_handler(const vector_t *strvec) {
  if (!strvec) return;

  use_pid_dir = true;
}

static void script_user_handler(const vector_t *strvec) {
  if (vector_size(strvec) < 2) {
    report_config_error(CONFIG_GENERAL_ERROR, "No script username specified");
    return;
  }

  if (set_default_script_user(
          strvec_slot(strvec, 1), vector_size(strvec) > 2 ? strvec_slot(strvec, 2) : NULL))
    report_config_error(CONFIG_GENERAL_ERROR, "Error setting global script uid/gid");
}

static void script_security_handler(__attribute__((unused)) const vector_t *strvec) {
  script_security = true;
}

static void child_wait_handler(const vector_t *strvec) {
  unsigned secs;
...
#ifdef _WITH_VRRP_
static void vrrp_rx_bufs_policy_handler(const vector_t *strvec) {
  ...
  for (i = 1; i < vector_size(strvec); i++) {
    if (!strcasecmp(strvec_slot(strvec, i), "MTU"))
      ...
      if (!read_unsigned_strvec(strvec, 1, &rx_buf_size, 0, UINT_MAX, false))
        report_config_error(
            CONFIG_GENERAL_ERROR, "Invalid vrrp_rx_bufs_policy %s", strvec_slot(strvec, i));
      else {
  ...
  if ((global_data->vrrp_rx_bufs_policy & RX_BUFS_SIZE) &&
    ...
    report_config_error(
        CONFIG_GENERAL_ERROR, "Cannot set vrrp_rx_bufs_policy size and policy, ignoring policy");
    global_data->vrrp_rx_bufs_policy &= ~(RX_BUFS_POLICY_MTU | RX_BUFS_POLICY_ADVERT);
  } else if (
      (global_data->vrrp_rx_bufs_policy & RX_BUFS_POLICY_MTU) &&
      (global_data->vrrp_rx_bufs_policy & RX_BUFS_POLICY_ADVERT)) {
    report_config_error(
        CONFIG_GENERAL_ERROR,
        "Cannot set both vrrp_rx_bufs_policy MTU and ADVERT, ignoring ADVERT");
    global_data->vrrp_rx_bufs_policy &= ~RX_BUFS_POLICY_ADVERT;
  }
}

static void vrrp_rx_bufs_multiplier_handler(const vector_t *strvec) {
  ...
  if (!read_unsigned_strvec(strvec, 1, &rx_buf_mult, 1, UINT_MAX, false))
    report_config_error(
        CONFIG_GENERAL_ERROR, "Invalid vrrp_rx_bufs_multiplier %s", strvec_slot(strvec, 1));
  else
...
static unsigned get_netlink_rcv_bufs_size(const vector_t *strvec, const char *type) {
  unsigned val;
  ...
  if (!read_unsigned_strvec(strvec, 1, &val, 0, UINT_MAX, false)) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "%s_rcv_bufs size (%s) invalid", type, strvec_slot(strvec, 1));
    return 0;
  }
...
#ifdef _WITH_VRRP_
static void vrrp_netlink_monitor_rcv_bufs_handler(const vector_t *strvec) {
  unsigned val;
  ...
  if (val) global_data->vrrp_netlink_monitor_rcv_bufs = val;
}

static void vrrp_netlink_monitor_rcv_bufs_force_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR,
          "Invalid value '%s' for global vrrp_netlink_monitor_rcv_bufs_force specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void vrrp_netlink_cmd_rcv_bufs_handler(const vector_t *strvec) {
  unsigned val;
  ...
  if (val) global_data->vrrp_netlink_cmd_rcv_bufs = val;
}

static void vrrp_netlink_cmd_rcv_bufs_force_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR,
          "Invalid value '%s' for global vrrp_netlink_cmd_rcv_bufs_force specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void process_monitor_rcv_bufs_handler(const vector_t *strvec) {
  unsigned val;
  ...
  if (val) global_data->process_monitor_rcv_bufs = val;
}

static void process_monitor_rcv_bufs_force_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR,
          "Invalid value '%s' for global process_monitor_rcv_bufs_force specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
#ifdef _WITH_LVS_
static void lvs_netlink_monitor_rcv_bufs_handler(const vector_t *strvec) {
  unsigned val;
  ...
  if (val) global_data->lvs_netlink_monitor_rcv_bufs = val;
}

static void lvs_netlink_monitor_rcv_bufs_force_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR,
          "Invalid value '%s' for global lvs_netlink_monitor_rcv_bufs_force specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void lvs_netlink_cmd_rcv_bufs_handler(const vector_t *strvec) {
  unsigned val;
  ...
  if (val) global_data->lvs_netlink_cmd_rcv_bufs = val;
}

static void lvs_netlink_cmd_rcv_bufs_force_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR,
          "Invalid value '%s' for global lvs_netlink_cmd_rcv_bufs_force specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void rs_init_notifies_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value '%s' for global rs_init_notifies specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void no_checker_emails_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value '%s' for global no_checker_emails specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
static void umask_handler(const vector_t *strvec) {
  long umask_long;
  mode_t umask_bits = 0;
  ...
  if (isdigit(mask[0])) {
    if (vector_size(strvec) != 2) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "%s parameter(s) to umask option",
          vector_size(strvec) == 1 ? "Missing" : "Extra");
      return;
    }
    ...
    for (i = 1; i < vector_size(strvec); i++) {
      for (p = strvec_slot(strvec, i); *p;) {
        if (need_or) {
          ...
          report_config_error(
              CONFIG_GENERAL_ERROR, "Invalid umask syntax %s", strvec_slot(strvec, i));
          return;
        }

        if (!strncmp(p, "IRUSR", 5))
          umask_bits |= S_IRUSR;
        else if (!strncmp(p, "IWUSR", 5))
          umask_bits |= S_IWUSR;
        else if (!strncmp(p, "IXUSR", 5))
          umask_bits |= S_IXUSR;
        else if (!strncmp(p, "IRWXU", 5))
          umask_bits |= S_IRWXU;
        else if (!strncmp(p, "IRGRP", 5))
          umask_bits |= S_IRGRP;
        else if (!strncmp(p, "IWGRP", 5))
          umask_bits |= S_IWGRP;
        else if (!strncmp(p, "IXGRP", 5))
          umask_bits |= S_IXGRP;
        else if (!strncmp(p, "IRWXG", 5))
          umask_bits |= S_IRWXG;
        else if (!strncmp(p, "IROTH", 5))
          umask_bits |= S_IROTH;
        else if (!strncmp(p, "IWOTH", 5))
          umask_bits |= S_IWOTH;
        else if (!strncmp(p, "IXOTH", 5))
          umask_bits |= S_IXOTH;
        else if (!strncmp(p, "IRWXO", 5))
          umask_bits |= S_IRWXO;
        else {
...
#ifdef _WITH_VRRP_
static void vrrp_startup_delay_handler(const vector_t *strvec) {
  unsigned startup_delay;

  if (!read_decimal_unsigned_strvec(
          strvec, 1, &startup_delay, TIMER_HZ / 1000, UINT_MAX, TIMER_HZ_DIGITS, true))
    report_config_error(
        CONFIG_GENERAL_ERROR, "vrrp_startup_delay '%s' is invalid", strvec_slot(strvec, 1));
  else
    global_data->vrrp_startup_delay = startup_delay;

  if (global_data->vrrp_startup_delay >= 60 * TIMER_HZ)
    log_message(
        LOG_INFO, "The vrrp_startup_delay is very large - %s seconds", strvec_slot(strvec, 1));
}

static void vrrp_log_unknown_vrids_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->log_unknown_vrids = true;
}

#ifdef _HAVE_VRRP_VMAC_
static void vrrp_vmac_prefix_handler(const vector_t *strvec) {
  if (global_data->vmac_prefix) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vmac prefix has already been specified - ignoring %s",
        strvec_slot(strvec, 1));
    return;
  }
...
static void vrrp_vmac_addr_prefix_handler(const vector_t *strvec) {
  if (global_data->vmac_addr_prefix) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "vmac_addr prefix has already been specified - ignoring %s",
        strvec_slot(strvec, 1));
    return;
  }
...
static void random_seed_handler(const vector_t *strvec) {
  unsigned val;
...
#ifndef _ONE_PROCESS_DEBUG_
static void include_check_handler(const vector_t *strvec) { include_check_set(strvec); }

static const char *set_dir(const char *dir_name) {
  char *save_dir = STRDUP(dir_name);
  size_t end;

  /* Remove a trailing / */
  end = strlen(save_dir);
  if (end && save_dir[end - 1] == '/') save_dir[end - 1] = '\0';

  return save_dir;
}

static void config_save_dir_handler(const vector_t *strvec) {
  struct stat statbuf;
  ...
  if (ret && errno == ENOENT) {
    ...
    if (!mkdir(dir_name, S_IRWXU))
      ...
      report_config_error(
          CONFIG_GENERAL_ERROR, "Unable to create config_save_dir %s (error %d - %m)", dir_name,
          errno);
  } else if (ret)
    report_config_error(CONFIG_GENERAL_ERROR, "config_save_dir %s error %d - %m", dir_name, errno);
  else
    report_config_error(
        CONFIG_GENERAL_ERROR, "config_save_dir %s exists and is not a directory", dir_name);
}

static void reload_check_config_handler(const vector_t *strvec) {
  if (vector_size(strvec) >= 2) {
...
static void reload_time_file_handler(const vector_t *strvec) {
  if (vector_size(strvec) != 2) {
...
static void reload_repeat_handler(__attribute__((unused)) const vector_t *strvec) {
  global_data->reload_repeat = true;
}

static void reload_file_handler(const vector_t *strvec) {
...
static void config_copy_directory_handler(const vector_t *strvec) {
  if (global_data->config_directory) {
    report_config_error(
        CONFIG_GENERAL_ERROR, "%s already specified - ignoring", strvec_slot(strvec, 0));
    return;
  }
...
static void data_use_instance_handler(const vector_t *strvec) {
  int res = true;
  ...
  if (vector_size(strvec) >= 2) {
    ...
    if (res < 0) {
      report_config_error(
          CONFIG_GENERAL_ERROR, "Invalid value '%s' for global date_use_instance specified",
          strvec_slot(strvec, 1));
      return;
    }
  }
...
#ifdef _WITH_JSON_
static void json_version_handler(const vector_t *strvec) {
...
void init_global_keywords(bool global_active) {
  ...
  install_keyword(
      "vrrp_vmac_garp_intvl",
      &vrrp_vmac_garp_extra_if_handler); /* Deprecated after v2.2.2 - incorrect keyword in commit
                                            3dcd13c */
#endif
  ...
  install_keyword(
      "vrrp_netlink_monitor_rcv_bufs_force", &vrrp_netlink_monitor_rcv_bufs_force_handler);
#ifdef _WITH_TRACK_PROCESS_
  ...
  install_keyword(
      "lvs_netlink_monitor_rcv_bufs_force", &lvs_netlink_monitor_rcv_bufs_force_handler);
