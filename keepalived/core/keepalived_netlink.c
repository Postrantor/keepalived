...
#ifdef _NETLINK_TIMERS_
void report_and_clear_netlink_timers(const char *str) {
  int i;
  ...
  for (i = 0; i <= MAX_NETLINK_TIMER; i++) {
    if (netlink_count[i]) {
      log_message(
          LOG_INFO, "  netlink cmd %d (%u calls), time %ld.%6.6ld", i, netlink_count[i],
          netlink_times[i].tv_sec, netlink_times[i].tv_usec);
...
static const char *get_nl_msg_type(unsigned type) {
  switch (type) {
...
static inline bool addr_is_equal2(
    struct ifaddrmsg *ifa, void *addr, ip_address_t *vip_addr, interface_t *ifp, vrrp_t *vrrp) {
  ...
  if (vip_addr->ifa.ifa_family != ifa->ifa_family) return false;
  if (vip_addr->ifp != ifp &&
...
static inline bool addr_is_equal(
    struct ifaddrmsg *ifa, void *addr, ip_address_t *vip_addr, interface_t *ifp) {
...
address_is_ours(struct ifaddrmsg *ifa, struct in_addr *addr, interface_t *ifp) {
  ...
  list_for_each_entry(top, &ifp->tracking_vrrp, e_list) {
    ...
    if (vrrp->state != VRRP_STATE_MAST) continue;

    for (vip_list = ifa->ifa_family == vrrp->family ? &vrrp->vip : &vrrp->evip; vip_list;
         vip_list = vip_list == &vrrp->vip ? &vrrp->evip : NULL) {
      list_for_each_entry(ip_addr, vip_list, e_list) {
        if (addr_is_equal(ifa, addr, ip_addr, ifp)) return ip_addr->dont_track ? NULL : vrrp;
      }
    }
  }
...
static bool __attribute__((pure)) ignore_address_if_ours_or_link_local(
    struct ifaddrmsg *ifa, struct in_addr *addr, interface_t *ifp) {
  tracking_obj_t *top;
  vrrp_t *vrrp;
  ip_address_t *ip_addr;

  /* We are only interested in link local for IPv6 */
  if (ifa->ifa_family == AF_INET6 && ifa->ifa_scope != RT_SCOPE_LINK) return true;
  ...
  list_for_each_entry(top, &ifp->tracking_vrrp, e_list) {
    ...
    if (ifa->ifa_family == vrrp->family) {
      list_for_each_entry(ip_addr, &vrrp->vip, e_list) {
        if (addr_is_equal2(ifa, addr, ip_addr, ifp, vrrp)) return true;
      }
    }
...
#ifdef _WITH_VRRP_
static bool compare_addr(int family, void *addr1, ip_address_t *addr2) {
  union {
    struct in_addr *in;
    struct in6_addr *in6;
  } addr1_p = {.in = addr1};

  if (family == AF_INET) return addr1_p.in->s_addr != addr2->u.sin.sin_addr.s_addr;
...
static ip_route_t *route_is_ours(
    struct rtmsg *rt, struct rtattr *tb[RTA_MAX + 1], vrrp_t **ret_vrrp) {
  uint32_t table;
  int family;
  ...
  *ret_vrrp = NULL;
  ...
  if (tb[RTA_PRIORITY]) priority = *PTR_CAST(uint32_t, RTA_DATA(tb[RTA_PRIORITY]));

  list_for_each_entry(vrrp, &vrrp_data->vrrp, e_list) {
    list_for_each_entry(route, &vrrp->vroutes, e_list) {
      if (table != route->table || family != route->family ||
          mask_len != route->dst->ifa.ifa_prefixlen || priority != route->metric ||
          tos != route->tos)
        continue;
      ...
      if (!tb[RTA_DST]) memset(&default_addr, 0, sizeof(default_addr));
  ...
  list_for_each_entry(route, &vrrp_data->static_routes, e_list) {
    if (table != route->table || family != route->family ||
        mask_len != route->dst->ifa.ifa_prefixlen || tos != route->tos)
      continue;
...
static bool compare_rule(
    struct fib_rule_hdr *frh, struct rtattr *tb[FRA_MAX + 1], ip_rule_t *rule) {
  if (rule->dont_track) return false;

  if (rule->family != frh->family) return false;

  /* This is a very good descriminator, since our rules will always have a priority */
  if (!tb[FRA_PRIORITY] || rule->priority != *PTR_CAST(uint32_t, RTA_DATA(tb[FRA_PRIORITY])))
    return false;

  if (frh->action != rule->action) return false;

  if (frh->action == FR_ACT_GOTO &&
      (!tb[FRA_GOTO] || *PTR_CAST(uint32_t, RTA_DATA(tb[FRA_GOTO])) != rule->goto_target))
    return false;

  if (tb[FRA_TABLE] && rule->table != *PTR_CAST(uint32_t, RTA_DATA(tb[FRA_TABLE]))) return false;
  if (!tb[FRA_TABLE] && rule->table != frh->table) return false;

  if (!rule->invert != !((frh->flags & FIB_RULE_INVERT))) return false;

  if (!rule->from_addr != !tb[FRA_SRC]) return false;
  if (rule->from_addr) {
    if (frh->src_len != rule->from_addr->ifa.ifa_prefixlen) return false;
    if (compare_addr(rule->family, RTA_DATA(tb[FRA_SRC]), rule->from_addr)) return false;
  }

  if (!rule->to_addr != !tb[FRA_DST]) return false;
  if (rule->to_addr) {
    if (frh->dst_len != rule->to_addr->ifa.ifa_prefixlen) return false;
    if (compare_addr(rule->family, RTA_DATA(tb[FRA_DST]), rule->to_addr)) return false;
  }

  if (rule->tos != frh->tos) return false;

  if (!tb[FRA_FWMARK] != !(rule->mask & IPRULE_BIT_FWMARK)) return false;
  if (rule->mask & IPRULE_BIT_FWMARK &&
      *PTR_CAST(uint32_t, RTA_DATA(tb[FRA_FWMARK])) != rule->fwmark)
    return false;

  if (!tb[FRA_FWMASK] && (rule->mask & IPRULE_BIT_FWMASK)) return false;
  if (rule->mask & IPRULE_BIT_FWMASK) {
    if (*PTR_CAST(uint32_t, RTA_DATA(tb[FRA_FWMASK])) != rule->fwmask) return false;
  } else if (tb[FRA_FWMASK]) {
    if (*PTR_CAST(uint32_t, RTA_DATA(tb[FRA_FWMASK])) != 0xffffffff) return false;
  }

  if (!tb[FRA_FLOW] != !rule->realms) return false;
  if (rule->realms && *PTR_CAST(uint32_t, RTA_DATA(tb[FRA_FLOW])) != rule->realms) return false;

#if HAVE_DECL_FRA_SUPPRESS_PREFIXLEN
  if (!tb[FRA_SUPPRESS_PREFIXLEN]) {
    if (rule->suppress_prefix_len != -1) return false;
  } else if (*PTR_CAST(int32_t, RTA_DATA(tb[FRA_SUPPRESS_PREFIXLEN])) != rule->suppress_prefix_len)
    return false;
#endif

#if HAVE_DECL_FRA_SUPPRESS_IFGROUP
  if (!tb[FRA_SUPPRESS_IFGROUP] != !(rule->mask & IPRULE_BIT_SUP_GROUP)) return false;
  if (rule->mask & IPRULE_BIT_SUP_GROUP &&
      *PTR_CAST(uint32_t, RTA_DATA(tb[FRA_SUPPRESS_IFGROUP])) != rule->suppress_group)
    return false;
#endif

  if (!tb[FRA_IFNAME] != !(rule->iif)) return false;
  if (rule->iif && strcmp(RTA_DATA(tb[FRA_IFNAME]), rule->iif->ifname)) return false;

  if (!tb[FRA_OIFNAME] != !(rule->oif)) return false;
  if (rule->oif && strcmp(RTA_DATA(tb[FRA_OIFNAME]), rule->oif->ifname)) return false;

#if HAVE_DECL_FRA_TUN_ID
  uint64_t tunnel_id;
  if (!tb[FRA_TUN_ID] != !(rule->tunnel_id)) return false;
  if (rule->tunnel_id) {
    tunnel_id = be64toh(*PTR_CAST(uint64_t, RTA_DATA(tb[FRA_TUN_ID])));
    if (tunnel_id != rule->tunnel_id) return false;
  }
#endif

#if HAVE_DECL_FRA_UID_RANGE
  if (!tb[FRA_UID_RANGE] != !(rule->mask & IPRULE_BIT_UID_RANGE)) return false;
  ...
  if (!tb[FRA_L3MDEV] && rule->l3mdev) return false;
  if (tb[FRA_L3MDEV] && *PTR_CAST(uint8_t, RTA_DATA(tb[FRA_L3MDEV])) != rule->l3mdev) return false;
#endif

#if HAVE_DECL_FRA_IP_PROTO
  if (!tb[FRA_IP_PROTO] != !(rule->mask & IPRULE_BIT_IP_PROTO)) return false;
  ...
  if (!tb[FRA_SPORT_RANGE] != !(rule->mask & IPRULE_BIT_SPORT_RANGE)) return false;
  ...
  if (!tb[FRA_DPORT_RANGE] != !(rule->mask & IPRULE_BIT_DPORT_RANGE)) return false;
...
static ip_rule_t *rule_is_ours(
    struct fib_rule_hdr *frh, struct rtattr *tb[FRA_MAX + 1], vrrp_t **ret_vrrp) {
  vrrp_t *vrrp;
  ip_rule_t *rule;
  ...
  *ret_vrrp = NULL;
  ...
  list_for_each_entry(rule, &vrrp_data->static_rules, e_list) {
    if (compare_rule(frh, tb, rule)) return rule;
  }
...
static void netlink_set_rx_buf_size(nl_handle_t *nl, unsigned rcvbuf_size, bool force) {
  if (!rcvbuf_size) rcvbuf_size = IF_DEFAULT_BUFSIZE;
...
#ifdef _WITH_VRRP_
static void kernel_netlink_set_membership(int group, bool add) {
  if (setsockopt(
          nl_kernel.fd, SOL_NETLINK, add ? NETLINK_ADD_MEMBERSHIP : NETLINK_DROP_MEMBERSHIP, &group,
          sizeof(group)) < 0)
    log_message(
        LOG_INFO, "Netlink: Cannot add membership on netlink socket : (%s)", strerror(errno));
}

void set_extra_netlink_monitoring(
    bool ipv4_routes, bool ipv6_routes, bool ipv4_rules, bool ipv6_rules) {
...
static void netlink_socket(
    nl_handle_t *nl, unsigned rcvbuf_size, bool force, int flags, unsigned group, ...) {
  int ret;
  va_list gp;
  ...
  if (nl->fd < 0) {
    log_message(LOG_INFO, "Netlink: Cannot open netlink socket : (%s)", strerror(errno));
    return;
  }
  ...
  if (ret < 0) {
    log_message(LOG_INFO, "Netlink: Cannot bind netlink socket : (%s)", strerror(errno));
    close(nl->fd);
    nl->fd = -1;
    return;
  }
  ...
  while (group) {
    ...
    if (ret < 0)
      log_message(
          LOG_INFO, "Netlink: Cannot add group %u membership on netlink socket : (%s)", group,
          strerror(errno));
  ...
  if (ret < 0 || addr_len != sizeof(snl)) {
    log_message(LOG_INFO, "Netlink: Cannot getsockname : (%s)", strerror(errno));
    close(nl->fd);
    nl->fd = -1;
    return;
  }
...
static void netlink_close(nl_handle_t *nl) {
  if (!nl) return;
...
addattr_l(struct nlmsghdr *n, size_t maxlen, unsigned short type, const void *data, size_t alen) {
  ...
  if (n->nlmsg_len + align_len > maxlen) return -1;
...
int addraw_l(struct nlmsghdr *n, size_t maxlen, const void *data, size_t len) {
  size_t align_len = NLMSG_ALIGN(len);

  if (n->nlmsg_len + align_len > maxlen) return -1;

  memcpy(NLMSG_TAIL(n), data, len);
  if (align_len > len) memset(PTR_CAST(char, NLMSG_TAIL(n)) + len, 0, align_len - len);
  n->nlmsg_len += (uint32_t)align_len;
  return 0;
}

size_t rta_addattr_l(
    struct rtattr *rta, size_t maxlen, unsigned short type, const void *data, size_t alen) {
  struct rtattr *subrta;
  size_t len = RTA_LENGTH(alen);
  size_t align_len = RTA_ALIGN(len);

  if (rta->rta_len + align_len > maxlen) return 0;
...
struct rtattr *rta_nest(struct rtattr *rta, size_t maxlen, unsigned short type) {
...
size_t rta_nest_end(struct rtattr *rta, struct rtattr *nest) {
...
static void parse_rtattr(struct rtattr **tb, int max, struct rtattr *rta, size_t len) {
...
static void set_vrrp_backup(vrrp_t *vrrp) {
  vrrp_t *isync;
...
have_address(const void *addr_p, const interface_t *ifp, int family) {
  ...
  if (!inet_inaddrcmp(
          family, addr_p,
          family == AF_INET ? (const void *)&ifp->sin_addr : (const void *)&ifp->sin6_addr))
    return true;

  addr_l = family == AF_INET ? &ifp->sin_addr_l : &ifp->sin6_addr_l;
  list_for_each_entry(addr, addr_l, e_list) {
    if (!inet_inaddrcmp(family, addr_p, addr)) return true;
  }
 ...
 */
static int netlink_if_address_filter(
    __attribute__((unused)) struct sockaddr_nl *snl, struct nlmsghdr *h) {
  ...
  if (h->nlmsg_len < NLMSG_LENGTH(sizeof(struct ifaddrmsg))) return -1;
  ...
  if (tb[IFA_LOCAL] == NULL) tb[IFA_LOCAL] = tb[IFA_ADDRESS];
...
#endif
  {
    ...
    // do we want to set a flag to say it is a generated link local address (or set saddr and
    // track_saddr, but not saddr_from_config) or can we just compare address to vrrp->ifp->base_ifp
    // address. We still need to consider non-vmac IPv6 if interface doesn't have a link local
    // address.
    if (h->nlmsg_type == RTM_NEWADDR) {
      if (!ignore_address_if_ours_or_link_local(ifa, addr.addr, ifp)) {
        /* The following code doesn't seem right.
         * 1. If using unicast, and the peers and not link local, then we need a non link-local
         * address
         * 2. There is an argument that we should process this if vrrp->base_priority ==
         * VRRP_PRIO_OWNER
        ...
        if (ifa->ifa_family == AF_INET) {
          if (!ifp->sin_addr.s_addr) {
            ...
            if (!list_empty(&ifp->tracking_vrrp)) addr_chg = true;
          } else {
        ...
        if (addr_chg) {
          if (__test_bit(LOG_DETAIL_BIT, &debug)) {
            ...
            log_message(LOG_INFO, "Assigned address %s for interface %s", addr_str, ifp->ifname);
          }
          ...
          list_for_each_entry(top, &ifp->tracking_vrrp, e_list) {
...
#endif
                                                   vrrp->ifp) &&
                vrrp->num_script_if_fault && vrrp->family == ifa->ifa_family &&
             ...
             * created one ourselves, add/replace the new address on the vmac */
                ...
                vrrp->family == AF_INET6 && vrrp->ifp && ifp == vrrp->ifp->base_ifp &&
                IS_MAC_IP_VLAN(vrrp->ifp) && !__test_bit(VRRP_VMAC_XMITBASE_BIT, &vrrp->flags) &&
                ifa->ifa_family == AF_INET6 && vrrp->ifp->is_ours) {
              inet_ip6tosockaddr(addr.in6, &vrrp->saddr);
#if 0
      ...
      if (ifa->ifa_family == AF_INET) {
        if (inaddr_equal(AF_INET, &ifp->sin_addr, addr.in)) {
          if (list_empty(&ifp->sin_addr_l))
            ...
            list_for_each_entry(top, &ifp->tracking_vrrp, e_list) {
              vrrp = top->obj.vrrp;
              if (VRRP_CONFIGURED_IFP(vrrp) != ifp) continue;
      ...
      if (addr_chg && !list_empty(&ifp->tracking_vrrp)) {
        if (__test_bit(LOG_DETAIL_BIT, &debug)) {
          inet_ntop(ifa->ifa_family, addr.addr, addr_str, sizeof(addr_str));
          log_message(LOG_INFO, "Deassigned address %s from interface %s", addr_str, ifp->ifname);
        }
        if (ifa->ifa_family == AF_INET)
        ...
        list_for_each_entry(top, &ifp->tracking_vrrp, e_list) {
          ...
          if (ifp != vrrp->ifp
            ...
            continue;
          if (vrrp->family != ifa->ifa_family) continue;
          if (!inaddr_equal(
                  ifa->ifa_family,
                  vrrp->family == AF_INET
                      ? &(PTR_CAST(struct sockaddr_in, &vrrp->saddr))->sin_addr
                      : (void *)&(PTR_CAST(struct sockaddr_in6, &vrrp->saddr))->sin6_addr,
                  addr.addr))
            continue;
          ...
          if (ifa->ifa_family == AF_INET6 && __test_bit(VRRP_VMAC_BIT, &vrrp->flags) &&
              ifp == vrrp->ifp->base_ifp && !__test_bit(VRRP_VMAC_XMITBASE_BIT, &vrrp->flags) &&
              !__test_bit(VRRP_FLAG_SADDR_FROM_CONFIG, &vrrp->flags)) {
            // This is rubbish if base i/f addr changed. Check against address generated from base
            // i/f's MAC
...
#endif
                                                     vrrp->ifp) &&
                  vrrp->family == ifa->ifa_family && vrrp->saddr.ss_family != AF_UNSPEC &&
                  (vrrp->family !=
                       AF_INET6 || /* For an IPv6 VMAC if down removes the link local address */
                   !__test_bit(VRRP_VMAC_BIT, &vrrp->flags) ||
                   __test_bit(VRRP_VMAC_XMITBASE_BIT, &vrrp->flags) || IF_ISUP(ifp)) &&
                  (!__test_bit(VRRP_FLAG_SADDR_FROM_CONFIG, &vrrp->flags) || is_tracking_saddr)) {
            down_instance(vrrp);
            vrrp->saddr.ss_family = AF_UNSPEC;
          }
    ...
    if (!addr_chg || list_empty(&ifp->tracking_vrrp)) {
      ...
      if (
        ...
        inet_ntop(ifa->ifa_family, addr.addr, addr_str, sizeof(addr_str));
        log_message(
            LOG_INFO, "Netlink reflector reports IP %s %s %s", addr_str,
            h->nlmsg_type == RTM_NEWADDR ? "added to" : "removed from", ifp->ifname);
      }
...
#endif
  {
    ...
    update_checker_activity(ifa->ifa_family, addr.addr, (h->nlmsg_type == RTM_NEWADDR));
  }
#endif
...
static int netlink_parse_info(
  ...
  while (true) {
    ...
    if (len < 0) {
      if (check_EAGAIN(errno)) break;
      if (errno == ENOBUFS) {
        log_message(
            LOG_INFO, "Netlink: Receive buffer overrun on %s socket - (%m)",
        ...
        log_message(
            LOG_INFO, "Netlink: recvmsg error on %s socket  - %d (%m)",
    ...
    if (msg.msg_namelen != sizeof snl) {
      log_message(LOG_INFO, "Netlink: Sender address length error: length %u", msg.msg_namelen);
      ret = -1;
      break;
    }

    /* See -Wcast-align comment above, also applies to NLMSG_NEXT */
    for (h = PTR_CAST(struct nlmsghdr, nlmsg_buf); NLMSG_OK(h, (size_t)len);
         h = NLMSG_NEXT(h, len)) {
      /* Finish off reading. */
      ...
      if (h->nlmsg_type == NLMSG_ERROR) {
        ...
        if (n && (err->error == -EEXIST) &&
            ((n->nlmsg_type == RTM_NEWROUTE) || (n->nlmsg_type == RTM_NEWADDR))) {
          FREE(nlmsg_buf);
          return 0;
        }
        ...
        if (n && err->error == -EADDRNOTAVAIL && n->nlmsg_type == RTM_DELADDR) {
...
#endif
          log_message(
              LOG_INFO, "Netlink: error: %s(%d), type=%s(%u), seq=%u, pid=%u",
              strerror(-err->error), -err->error, get_nl_msg_type(err->msg.nlmsg_type),
...
#endif
          h->nlmsg_type != RTM_NEWLINK && h->nlmsg_type != RTM_DELLINK &&
...
#ifdef _WITH_VRRP_
/* Out talk filter */
static int netlink_talk_filter(
    __attribute__((unused)) struct sockaddr_nl *snl, struct nlmsghdr *h) {
...
ssize_t netlink_talk(nl_handle_t *nl, struct nlmsghdr *n) {
  ssize_t status;
  struct sockaddr_nl snl;
  ...
  if (status < 0) {
    log_message(
        LOG_INFO, "Netlink: sendmsg(%d) cmd %d error: %s", nl->fd, n->nlmsg_type, strerror(errno));
    return -1;
  }
...
static int netlink_request(
  ...
  status =
      sendto(nl->fd, (void *)&req, sizeof(req), 0, PTR_CAST(struct sockaddr, &snl), sizeof(snl));
  if (status < 0) {
    log_message(LOG_INFO, "Netlink: sendto() failed: %s", strerror(errno));
    return -1;
  }
  return 0;
}
...
#ifdef _WITH_VRRP_
void process_if_status_change(interface_t *ifp) {
  vrrp_t *vrrp;
  tracking_obj_t *top;
  bool now_up = FLAGS_UP(ifp->ifi_flags);

  if (now_up) ifp->seen_up = true;
...
static void process_interface_flags_change(interface_t *ifp, unsigned ifi_flags) {
  ...
  if (!list_empty(&ifp->tracking_vrrp)) {
    ...
    if (ifp->vmac_type && ifp->is_ours &&
        ...
        now_up && IN6_IS_ADDR_UNSPECIFIED(&ifp->sin6_addr)) {
      list_for_each_entry(top, &ifp->tracking_vrrp, e_list) {
        vrrp = top->obj.vrrp;
...
static void delayed_if_flags_change_thread(thread_ref_t thread) {
...
static void update_interface_flags(interface_t *ifp, unsigned ifi_flags, bool immediate) {
  bool was_up, now_up;
  ...
  if (ifp->flags_change_thread) {
    if (ifi_flags == ifp->flags_change_thread->u.uval) return;
  ...
  if (was_up == now_up) return;

  debounce_timer = now_up ? ifp->up_debounce_timer : ifp->down_debounce_timer;
  if (ifp->seen_up && debounce_timer && !immediate) {
    ifp->flags_change_thread = thread_add_timer_uval(
        master, delayed_if_flags_change_thread, ifp, ifi_flags, debounce_timer);
    log_message(
        LOG_INFO, "%s: Adding flags change from 0x%x to 0x%x delay by %u", ifp->ifname,
        ifp->ifi_flags, ifi_flags, debounce_timer);
    return;
  }
...
static const char *get_mac_string(int type) {
  switch (type) {
    case IFLA_BROADCAST:
      return "Broadcast";
    case IFLA_ADDRESS:
...
static bool netlink_if_get_ll_addr(interface_t *ifp, struct rtattr *tb[], int type, char *name) {
  size_t i;
  ...
  if (tb[type]) {
    ...
    if (hw_addr_len > sizeof(ifp->hw_addr)) {
      log_message(
          LOG_ERR, " %s MAC address for %s is too large: %zu", get_mac_string(type), name,
          hw_addr_len);
      return false;
    }
    ...
    switch (type) {
      ...
      case IFLA_BROADCAST:
        memcpy(ifp->hw_addr_bcast, RTA_DATA(tb[type]), hw_addr_len);
        break;
...
static void parse_af_spec(struct rtattr *attr, interface_t *ifp) {
  ...
  if (afspec[AF_INET]) {
    ...
    if (inet[IFLA_INET_CONF]) {
      ...
      if (ifp->rp_filter == UINT_MAX) ifp->rp_filter = inet_devconf[IPV4_DEVCONF_RP_FILTER - 1];
#endif
...
static bool netlink_if_link_populate(interface_t *ifp, struct rtattr *tb[], struct ifinfomsg *ifi) {
  char *name;
#ifdef _HAVE_VRRP_VMAC_
    ...
    if (linkinfo[IFLA_INFO_KIND]) {
      if (!strcmp((char *)RTA_DATA(linkinfo[IFLA_INFO_KIND]), "macvlan") ||
        ...
        parse_rtattr_nested(linkattr, IFLA_MACVLAN_MAX, linkinfo[IFLA_INFO_DATA]);
          ...
          !strcmp((char *)RTA_DATA(linkinfo[IFLA_INFO_KIND]), "ipvlan") ||
...
#ifdef _HAVE_IPV4_DEVCONF_
  if (tb[IFLA_AF_SPEC]) parse_af_spec(tb[IFLA_AF_SPEC], ifp);
#endif
  ...
  if (!global_data->allow_if_changes && ifp->seen_interface) {
    ...
    if (IS_MAC_IP_VLAN(ifp) != was_vlan) return false;
  ...
  if (!netlink_if_get_ll_addr(ifp, tb, IFLA_ADDRESS, name)) return false;
  if (!netlink_if_get_ll_addr(ifp, tb, IFLA_BROADCAST, name)) return false;
  ...
  if (!is_vrf_master) {
    if (tb[IFLA_MASTER]) {
      ...
      if (!ifp->vrf_master_ifp || new_vrf_master_index != ifp->vrf_master_ifp->ifindex) {
  ...
  if (FLAGS_UP(ifi->ifi_flags)) ifp->seen_up = true;

  return true;
}

/* Netlink interface link lookup filter */
static int netlink_if_link_filter(
    __attribute__((unused)) struct sockaddr_nl *snl, struct nlmsghdr *h) {
  ...
  if (h->nlmsg_len < NLMSG_LENGTH(sizeof(struct ifinfomsg))) return -1;
  ...
  if (tb[IFLA_IFNAME] == NULL) return -1;
  name = (char *)RTA_DATA(tb[IFLA_IFNAME]);
  ...
  /* Fill the interface structure */
  if (!netlink_if_link_populate(ifp, tb, ifi)) return -1;

  if (ifp->ifindex) update_interface_flags(ifp, ifi->ifi_flags, true);

  return 0;
}

/* Interfaces lookup bootstrap function */
int netlink_interface_lookup(char *name) {
  /* Interface lookup */
  if (netlink_request(&nl_cmd, AF_PACKET, RTM_GETLINK, name) < 0) return -1;
...
static int netlink_address_lookup(void) {
  ...
  /* IPv4 Address lookup */
  if (netlink_request(&nl_cmd, AF_INET, RTM_GETADDR, NULL) < 0) return -1;

  if ((status = netlink_parse_info(netlink_if_address_filter, &nl_cmd, NULL, false))) return status;

  /* IPv6 Address lookup */
  if (netlink_request(&nl_cmd, AF_INET6, RTM_GETADDR, NULL) < 0) return -1;
...
static int netlink_link_filter(
    __attribute__((unused)) struct sockaddr_nl *snl, struct nlmsghdr *h) {
  struct ifinfomsg *ifi;
  struct rtattr *tb[IFLA_MAX + 1];
  interface_t *ifp;
  ...
  if (!(h->nlmsg_type == RTM_NEWLINK || h->nlmsg_type == RTM_DELLINK)) return 0;
  ...
  if (!ifi->ifi_change && tb[IFLA_WIRELESS] && h->nlmsg_type == RTM_NEWLINK) return 0;
  ...
  if (ifp) {
    if (h->nlmsg_type == RTM_DELLINK) {
      if ((!list_empty(&ifp->tracking_vrrp)) || __test_bit(LOG_DETAIL_BIT, &debug))
      ...
      if (tb[IFLA_ADDRESS]) {
        ...
        if (ifp->hw_addr_len != hw_addr_len ||
            memcmp(ifp->hw_addr, RTA_DATA(tb[IFLA_ADDRESS]), hw_addr_len)) {
          if (hw_addr_len > sizeof(ifp->hw_addr)) {
            log_message(
                LOG_ERR, "MAC %s for %s is too large: %zu", get_mac_string(IFLA_ADDRESS),
                ifp->ifname, hw_addr_len);
          } else {
            if (__test_bit(LOG_DETAIL_BIT, &debug)) {
              if (!ifp->hw_addr_len)
            ...
            if (__test_bit(LOG_DETAIL_BIT, &debug)) {
              ...
              log_message(
                  LOG_INFO, "(%s) MAC %s changed from %s to %s", ifp->ifname,
                  get_mac_string(IFLA_ADDRESS), old_mac_buf, mac_buf);
            }
          }
        }
      }
      ...
      if (tb[IFLA_BROADCAST]) {
        ...
        if (ifp->hw_addr_len && ifp->hw_addr_len != hw_addr_len)
          log_message(
              LOG_ERR, "MAC broadcast address length %zu does not match MAC address length %zu",
              hw_addr_len, ifp->hw_addr_len);
        else if (memcmp(ifp->hw_addr_bcast, RTA_DATA(tb[IFLA_BROADCAST]), hw_addr_len)) {
          if (hw_addr_len > sizeof(ifp->hw_addr_bcast)) {
            log_message(
                LOG_ERR, "MAC %s for %s is too large: %zu", get_mac_string(IFLA_BROADCAST),
                ifp->ifname, hw_addr_len);
          } else {
            if (__test_bit(LOG_DETAIL_BIT, &debug))
            ...
            if (__test_bit(LOG_DETAIL_BIT, &debug)) {
              ...
              log_message(
                  LOG_INFO, "(%s) MAC %s changed from %s to %s", ifp->ifname,
                  get_mac_string(IFLA_BROADCAST), old_mac_buf, mac_buf);
            }
          }
        }
      }
      ...
      if (strcmp(ifp->ifname, name)) {
        ...
        if (ifp->is_ours
          ...
          if (ifp->base_ifp->ifindex) thread_add_event(master, recreate_vmac_thread, ifp, 0);
        } else
#endif
        ...
        if (
          ...
          old_mtu = ifp->mtu;
          ifp->mtu = *PTR_CAST(uint32_t, RTA_DATA(tb[IFLA_MTU]));
          if (old_mtu != ifp->mtu && !list_empty(&ifp->tracking_vrrp)) update_mtu(ifp);
        }
  ...
  if (!ifp) {
    if (h->nlmsg_type == RTM_NEWLINK) {
      ...
      if (prog_type == PROG_TYPE_VRRP)
#endif
        if (ifp->mtu > old_mtu) alloc_vrrp_buffer(ifp->mtu);
  ...
  update_interface_flags(
      ifp, (h->nlmsg_type == RTM_DELLINK) ? 0 : ifi->ifi_flags, immediate_change);

  return 0;
}

#ifdef _WITH_VRRP_
static int netlink_route_filter(
    __attribute__((unused)) struct sockaddr_nl *snl, struct nlmsghdr *h) {
  struct rtmsg *rt;
  ...
  if (h->nlmsg_len < NLMSG_LENGTH(sizeof(*rt))) return -1;
  ...
  /* Only IPv4 and IPv6 are valid for us */
  if (rt->rtm_family != AF_INET && rt->rtm_family != AF_INET6) return 0;
  ...
  if (!(route = route_is_ours(rt, tb, &vrrp))) return 0;
  ...
  if (h->nlmsg_type == RTM_NEWROUTE) {
    ...
    if (tb[RTA_OIF]) {
      ...
      if (route->oif && route->oif->ifindex != route->configured_ifindex)
        log_message(
            LOG_INFO, "route added index %" PRIu32 " != config index %u", route->configured_ifindex,
            route->oif->ifindex);
    } else
  ...
  if (route->dont_track) return 0;

  if (vrrp) {
    if (vrrp->state != VRRP_STATE_MAST) return 0;
...
static int netlink_rule_filter(
    __attribute__((unused)) struct sockaddr_nl *snl, struct nlmsghdr *h) {
  ...
  if (h->nlmsg_type != RTM_NEWRULE && h->nlmsg_type != RTM_DELRULE) return 0;

  if (h->nlmsg_len < NLMSG_LENGTH(sizeof(*frh))) return -1;

  frh = NLMSG_DATA(h);

  /* Only IPv4 and IPv6 are valid for us */
  if (frh->family != AF_INET && frh->family != AF_INET6) return 0;
  ...
  if (tb[FRA_PROTOCOL] && *PTR_CAST(uint8_t, RTA_DATA(tb[FRA_PROTOCOL])) != RTPROT_KEEPALIVED) {
  ...
  if (h->nlmsg_type != RTM_DELRULE) return 0;

  if (!(ip_rule = rule_is_ours(frh, tb, &vrrp))) return 0;

  ip_rule->set = false;

  if (ip_rule->dont_track) return 0;
...
static int netlink_broadcast_filter(struct sockaddr_nl *snl, struct nlmsghdr *h) {
  switch (h->nlmsg_type) {
    ...
    default:
      log_message(
          LOG_INFO, "Kernel is reflecting an unknown netlink nlmsg_type: %d", h->nlmsg_type);
      break;
  }
  return 0;
}

static void kernel_netlink(thread_ref_t thread) {
...
#ifdef _WITH_VRRP_
void kernel_netlink_poll(void) {
  if (nl_kernel.fd < 0) return;

  netlink_parse_info(netlink_broadcast_filter, &nl_kernel, NULL, true);
}
#endif

void kernel_netlink_set_recv_bufs(void) {
#ifdef _ONE_PROCESS_DEBUG_
#ifdef _WITH_VRRP_
  netlink_set_rx_buf_size(
      &nl_kernel, global_data->vrrp_netlink_monitor_rcv_bufs,
      global_data->vrrp_netlink_monitor_rcv_bufs_force);
  netlink_set_rx_buf_size(
      &nl_cmd, global_data->vrrp_netlink_cmd_rcv_bufs,
      global_data->vrrp_netlink_cmd_rcv_bufs_force);
#else
  netlink_set_rx_buf_size(
      &nl_kernel, global_data->lvs_netlink_monitor_rcv_bufs,
      global_data->lvs_netlink_monitor_rcv_bufs_force);
  netlink_set_rx_buf_size(
      &nl_cmd, global_data->lvs_netlink_cmd_rcv_bufs, global_data->lvs_netlink_cmd_rcv_bufs_force);
#endif
#else
#ifdef _WITH_VRRP_
  if (prog_type == PROG_TYPE_VRRP) {
...
void kernel_netlink_close_monitor(void) { netlink_close(&nl_kernel); }

void kernel_netlink_close_cmd(void) { netlink_close(&nl_cmd); }

void kernel_netlink_close(void) {
...
void kernel_netlink_init(void) {
  ...
  if (nl_kernel.fd >= 0) {
    nl_kernel.thread =
        thread_add_read(master, kernel_netlink, &nl_kernel, nl_kernel.fd, TIMER_NEVER, 0);
    return;
  }
  ...
  netlink_socket(
      &nl_kernel, global_data->vrrp_netlink_monitor_rcv_bufs,
      global_data->vrrp_netlink_monitor_rcv_bufs_force, SOCK_NONBLOCK, RTNLGRP_LINK,
      RTNLGRP_IPV4_IFADDR, RTNLGRP_IPV6_IFADDR, 0);
#else
  netlink_socket(
      &nl_kernel, global_data->lvs_netlink_monitor_rcv_bufs,
      global_data->lvs_netlink_monitor_rcv_bufs_force, SOCK_NONBLOCK, RTNLGRP_IPV4_IFADDR,
...
#ifdef _WITH_VRRP_
  netlink_socket(
      &nl_cmd, global_data->vrrp_netlink_cmd_rcv_bufs, global_data->vrrp_netlink_cmd_rcv_bufs_force,
      0, 0);
#else
  netlink_socket(
      &nl_cmd, global_data->lvs_netlink_cmd_rcv_bufs, global_data->lvs_netlink_cmd_rcv_bufs_force,
  ...
  if (prog_type == PROG_TYPE_VRRP)
    netlink_socket(
        &nl_cmd, global_data->vrrp_netlink_cmd_rcv_bufs,
        global_data->vrrp_netlink_cmd_rcv_bufs_force, 0, 0);
#endif
#ifdef _WITH_LVS_
  if (prog_type == PROG_TYPE_CHECKER)
    netlink_socket(
  ...
  if (prog_type == PROG_TYPE_CHECKER) kernel_netlink_close_cmd();
#endif
}

void cancel_kernel_netlink_threads(void) {
...
#ifdef _WITH_VRRP_
void kernel_netlink_read_interfaces(void) {
  int ret;
  ...
  if (nl_cmd.fd < 0) fprintf(stderr, "Error while registering Kernel netlink cmd channel\n");
...
#ifdef THREAD_DUMP
void register_keepalived_netlink_addresses(void) {
