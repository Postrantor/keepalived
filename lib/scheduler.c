...
#ifdef THREAD_DUMP
static const char *get_thread_type_str(thread_type_t id) {
...
static inline int function_cmp(const void *func, const struct rb_node *a) {
  if (func < (void *)rb_entry_const(a, func_det_t, n)->func) return -1;
  if (func > (void *)rb_entry_const(a, func_det_t, n)->func) return 1;
  return 0;
}

static inline bool function_less(struct rb_node *a, const struct rb_node *b) {
  return (rb_entry_const(a, func_det_t, n)->func < rb_entry_const(b, func_det_t, n)->func);
}

static const char *get_function_name(thread_func_t func) {
  struct rb_node *match_node;
  static char address[19];

  if (!RB_EMPTY_ROOT(&funcs)) {
    match_node = rb_find((void *)func, &funcs, function_cmp);
    if (match_node) return rb_entry(match_node, func_det_t, n)->name;
  }
...
const char *get_signal_function_name(void (*func)(void *, int)) {
...
void register_thread_address(const char *func_name, thread_func_t func) {
  func_det_t *func_det;
...
void deregister_thread_addresses(void) {
  ...
  if (RB_EMPTY_ROOT(&funcs)) return;

  rbtree_postorder_for_each_entry_safe(func_det, func_det_tmp, &funcs, n) FREE(func_det);
...
#ifdef _VRRP_FD_DEBUG_
void set_extra_threads_debug(void (*func)(void)) { extra_threads_debug = func; }
#endif
...
void register_shutdown_function(void (*func)(int)) {
...
static void thread_move_ready(
    thread_master_t *m, rb_root_cached_t *root, thread_t *thread, int type) {
  rb_erase_cached(&thread->n, root);
  INIT_LIST_HEAD(&thread->e_list);
  list_add_tail(&thread->e_list, &m->ready);
  if (thread->type != THREAD_TIMER_SHUTDOWN) thread->type = type;
}

/* Move ready thread into ready queue */
static void thread_rb_move_ready(thread_master_t *m, rb_root_cached_t *root, int type) {
  thread_t *thread;
  ...
  while ((thread_node = rb_first_cached(root))) {
    ...
    if (thread->sands.tv_sec == TIMER_DISABLED || timercmp(&time_now, &thread->sands, <)) break;
...
/* Update timer value */
static void thread_update_timer(rb_root_cached_t *root, timeval_t *timer_min) {
  const thread_t *first;
  rb_node_t *first_node;

  if (!(first_node = rb_first_cached(root))) return;

  first = rb_entry(first_node, thread_t, n);

  if (first->sands.tv_sec == TIMER_DISABLED) return;

  if (!timerisset(timer_min) || timercmp(&first->sands, timer_min, <=)) *timer_min = first->sands;
}

/* Compute the wait timer. Take care of timeouted fd */
static timeval_t thread_set_timer(thread_master_t *m) {
...
static void thread_timerfd_handler(thread_ref_t thread) {
  thread_master_t *m = thread->master;
  uint64_t expired;
  ssize_t len;

  len = read(m->timer_fd, &expired, sizeof(expired));
  if (len < 0) log_message(LOG_ERR, "scheduler: Error reading on timerfd fd:%d (%m)", m->timer_fd);
...
static inline int thread_child_pid_cmp(const void *pid, const rb_node_t *a) {
  return less_equal_greater_than(
      *PTR_CAST_CONST(pid_t, pid), rb_entry_const(a, thread_t, rb_data)->u.c.pid);
}

static inline bool thread_child_pid_less(rb_node_t *a, const rb_node_t *b) {
  return rb_entry(a, thread_t, rb_data)->u.c.pid < rb_entry_const(b, thread_t, rb_data)->u.c.pid;
}

void set_child_finder_name(char const *(*func)(pid_t)) { child_finder_name = func; }
...
char *const *get_cmd_line_options(int *argc) {
  *argc = sav_argc;
  return sav_argv;
}
...
void log_command_line(unsigned indent) {
  size_t len = 0;
  char *log_str;
  char *p;
  int i;
...
#ifndef _ONE_PROCESS_DEBUG_
unsigned calc_restart_delay(
    const timeval_t *last_start_time, unsigned *next_restart_delay, const char *name) {
  ...
  log_message(
      LOG_INFO, "Restart of %s process delayed %u seconds to limit respawn rate", name,
      restart_delay);

  return restart_delay;
}

void log_child_died(const char *process, pid_t pid) {
  ...
  log_message(
      LOG_INFO, "  copies of the keepalived system log entries around the time this happened");
}

/* report_child_status returns true if the exit is a hard error, so unable to continue */
int report_child_status(int status, pid_t pid, char const *prog_name) {
  ...
  if (WIFEXITED(status)) {
    ...
    if (exit_status == KEEPALIVED_EXIT_FATAL || exit_status == KEEPALIVED_EXIT_CONFIG ||
        exit_status == KEEPALIVED_EXIT_MISSING_PERMISSION) {
      log_message(
          LOG_INFO, "%s exited with permanent error %s. Terminating", prog_id,
    ...
    if (WTERMSIG(status) == SIGSEGV) {
      ...
      log_message(
          LOG_INFO, "  %s", "Please report a bug at https://github.com/acassen/keepalived/issues");
      log_message(
          LOG_INFO, "  %s", "and include this log from when keepalived started, a description");
      log_message(
          LOG_INFO, "  %s",
          "of what happened before the crash, your configuration file and the details below.");
      log_message(
          LOG_INFO, "  %s",
          "Also provide the output of keepalived -v, and whether keepalived is being");
      log_message(LOG_INFO, "  %s", "run in a container or VM.");
      log_message(
          LOG_INFO, "  %s",
          "A failure to provide all this information may mean the crash cannot be investigated.");
      log_message(
          LOG_INFO, "  %s",
          "If you are able to provide a stack backtrace with gdb that would really help.");
      log_message(
          LOG_INFO, "  Source version %s %s%s", PACKAGE_VERSION,
#ifdef GIT_COMMIT
      ...
      log_message(
          LOG_INFO, "  Built with kernel headers for Linux %d.%d.%d",
          (LINUX_VERSION_CODE >> 16) & 0xff, (LINUX_VERSION_CODE >> 8) & 0xff,
          (LINUX_VERSION_CODE)&0xff);
      uname(&uname_buf);
      log_message(
          LOG_INFO, "  Running on %s %s %s", uname_buf.sysname, uname_buf.release,
          uname_buf.version);
      log_command_line(2);
      ...
      log_message(
          LOG_INFO, "%s exited due to signal %d (%s)%s", prog_id, WTERMSIG(status),
...
/* epoll related */
static int thread_events_resize(thread_master_t *m, int delta) {
  unsigned int new_size;

  m->epoll_count += delta;
  if (m->epoll_count < m->epoll_size) return 0;

  new_size = ((m->epoll_count / THREAD_EPOLL_REALLOC_THRESH) + 1);
  new_size *= THREAD_EPOLL_REALLOC_THRESH;

  if (m->epoll_events) FREE(m->epoll_events);
...
static inline int thread_event_cmp(const void *key, const rb_node_t *a) {
  int fd = *PTR_CAST_CONST(int, key);

  return fd - rb_entry_const(a, thread_event_t, n)->fd;
}

static inline bool thread_event_less(rb_node_t *a, const rb_node_t *b) {
  return rb_entry(a, thread_event_t, n)->fd < rb_entry_const(b, thread_event_t, n)->fd;
}

static thread_event_t *thread_event_new(thread_master_t *m, int fd) {
...
static thread_event_t *__attribute__((pure)) thread_event_get(thread_master_t *m, int fd) {
  rb_node_t *node;
...
static int thread_event_set(const thread_t *thread) {
  thread_event_t *event = thread->event;
  thread_master_t *m = thread->master;
  struct epoll_event ev = {.events = 0, .data.ptr = event};
  int op;

  if (__test_bit(THREAD_FL_READ_BIT, &event->flags)) ev.events |= EPOLLIN;

  if (__test_bit(THREAD_FL_WRITE_BIT, &event->flags)) ev.events |= EPOLLOUT;
  ...
  if (epoll_ctl(m->epoll_fd, op, event->fd, &ev) < 0) {
    log_message(
        LOG_INFO, "scheduler: Error %d performing control on EPOLL instance for fd %d (%m)", errno,
        event->fd);
    return -1;
  }
...
static int thread_event_cancel(const thread_t *thread_cp) {
  ...
  if (event == m->current_event) m->current_event = NULL;
  thread_events_resize(m, -1);
  FREE(thread->event);
  return 0;
}

static int thread_event_del(const thread_t *thread_cp, unsigned flag) {
  thread_t *thread = no_const(thread_t, thread_cp);
  thread_event_t *event = thread->event;

  if (!__test_bit(flag, &event->flags)) return 0;

  if (flag == THREAD_FL_EPOLL_READ_BIT) {
    __clear_bit(THREAD_FL_READ_BIT, &event->flags);
    if (!__test_bit(THREAD_FL_EPOLL_WRITE_BIT, &event->flags)) return thread_event_cancel(thread);

    event->read = NULL;
  } else if (flag == THREAD_FL_EPOLL_WRITE_BIT) {
    __clear_bit(THREAD_FL_WRITE_BIT, &event->flags);
    if (!__test_bit(THREAD_FL_EPOLL_READ_BIT, &event->flags)) return thread_event_cancel(thread);

    event->write = NULL;
  }

  if (thread_event_set(thread) < 0) return -1;
...
thread_master_t *thread_make_master(void) {
  ...
  new->timer_thread =
      thread_add_read(new, thread_timerfd_handler, NULL, new->timer_fd, TIMER_NEVER, 0);
...
#ifdef THREAD_DUMP
static const char *timer_delay(timeval_t sands) {
  static char str[43];

  if (sands.tv_sec == TIMER_DISABLED) return "NEVER";
  if (sands.tv_sec == 0 && sands.tv_usec == 0) return "UNSET";
...
/* Dump rbtree */
static inline void write_thread_entry(FILE *fp, unsigned index, const thread_t *thread) {
  conf_write(
      fp,
      "#%.2u Thread:%p type %s, event %p, val/fd/pid %d, fd_flags %x, timer: %s, func %s(), id %lu",
      index, thread, get_thread_type_str(thread->type), thread->event, thread->u.val,
      thread->u.f.flags, timer_delay(thread->sands), get_function_name(thread->func), thread->id);
}

static void thread_rb_dump(const rb_root_cached_t *root, const char *tree, FILE *fp) {
  thread_t *thread;
  unsigned i = 1;

  conf_write(fp, "----[ Begin rb_dump %s ]----", tree);

  rb_for_each_entry_cached(thread, root, n) write_thread_entry(fp, i++, thread);

  conf_write(fp, "----[ End rb_dump ]----");
}

static void thread_list_dump(const list_head_t *l, const char *list_type, FILE *fp) {
  thread_t *thread;
  unsigned i = 1;

  conf_write(fp, "----[ Begin list_dump %s ]----", list_type);

  list_for_each_entry(thread, l, e_list) write_thread_entry(fp, i++, thread);

  conf_write(fp, "----[ End list_dump ]----");
}

static void event_rb_dump(const rb_root_t *root, const char *tree, FILE *fp) {
  thread_event_t *event;
  int i = 1;

  conf_write(fp, "----[ Begin rb_dump %s ]----", tree);
  rb_for_each_entry(event, root, n) conf_write(
      fp, "#%.2d event %p fd %d, flags: 0x%lx, read %p, write %p", i++, event, event->fd,
      event->flags, event->read, event->write);
  conf_write(fp, "----[ End rb_dump ]----");
}

void dump_thread_data(const thread_master_t *m, FILE *fp) {
...
static void thread_destroy_list(thread_master_t *m, list_head_t *l) {
  thread_t *thread, *thread_tmp;

  list_for_each_entry_safe(thread, thread_tmp, l, e_list) {
    /* The following thread types are relevant for the ready list */
    if (thread->type == THREAD_READY_READ_FD || thread->type == THREAD_READY_WRITE_FD ||
        thread->type == THREAD_READ_TIMEOUT || thread->type == THREAD_WRITE_TIMEOUT ||
        thread->type == THREAD_READ_ERROR || thread->type == THREAD_WRITE_ERROR) {
      ...
      if (thread->u.f.flags & THREAD_DESTROY_CLOSE_FD) thread_close_fd(thread);

      /* Do we need to free arg? */
      if (thread->u.f.flags & THREAD_DESTROY_FREE_ARG) FREE(thread->arg);
    }
...
static void thread_destroy_rb(thread_master_t *m, rb_root_cached_t *root) {
  thread_t *thread;
  thread_t *thread_sav;
...
/* Cleanup master */
void thread_cleanup_master(thread_master_t *m, bool keep_children) {
  ...
  if (!keep_children) thread_destroy_rb(m, &m->child);
  thread_destroy_list(m, &m->event);
#ifdef USE_SIGNAL_THREADS
  thread_destroy_list(m, &m->signal);
#endif
  thread_destroy_list(m, &m->ready);
  if (!keep_children) m->child_pid = RB_ROOT;
...
void thread_destroy_master(thread_master_t *m) {
  ...
  if (m->timer_fd != -1) close(m->timer_fd);

  if (m->signal_fd != -1) signal_handler_destroy();
...
static thread_t *thread_trim_head(list_head_t *l) {
  thread_t *thread;
...
static inline unsigned long thread_get_id(thread_master_t *m) { return m->id++; }

/* Make new thread. */
static thread_t *thread_new(thread_master_t *m) {
  thread_t *new;
...
/* Add new read thread. */
thread_ref_t thread_add_read_sands(
    thread_master_t *m,
    thread_func_t func,
    void *arg,
    int fd,
    const timeval_t *sands,
    unsigned flags) {
  thread_event_t *event;
  thread_t *thread;
  ...
  if (!event) {
    ...
    log_message(
        LOG_INFO, "scheduler: There is already read event %p (read %p) registered on fd [%d]",
        event, event->read, fd);
    return NULL;
  }
...
void thread_del_read(thread_ref_t thread) {
  if (!thread || !thread->event) return;

  thread_event_del(thread, THREAD_FL_EPOLL_READ_BIT);
}

#ifdef _WITH_SNMP_
static void thread_del_read_fd(thread_master_t *m, int fd) {
...
static void thread_read_requeue(thread_master_t *m, int fd, const timeval_t *new_sands) {
  thread_t *thread;
  thread_event_t *event;
...
void thread_requeue_read(thread_master_t *m, int fd, const timeval_t *sands) {
  thread_read_requeue(m, fd, sands);
}

/* Add new write thread. */
thread_ref_t thread_add_write(
    thread_master_t *m,
    thread_func_t func,
    void *arg,
    int fd,
...
void thread_del_write(thread_ref_t thread) {
  if (!thread || !thread->event) return;

  thread_event_del(thread, THREAD_FL_EPOLL_WRITE_BIT);
}

void thread_close_fd(thread_ref_t thread_cp) {
  thread_t *thread = no_const(thread_t, thread_cp);

  if (thread->u.f.fd == -1) return;

  if (thread->event) thread_event_cancel(thread);
...
thread_ref_t thread_add_timer_uval(
    thread_master_t *m, thread_func_t func, void *arg, unsigned val, unsigned long timer) {
  thread_t *thread;
...
thread_ref_t thread_add_timer(
    thread_master_t *m, thread_func_t func, void *arg, unsigned long timer) {
  return thread_add_timer_uval(m, func, arg, 0, timer);
}

void thread_update_arg2(thread_ref_t thread_cp, const thread_arg2 *u) {
  thread_t *thread = no_const(thread_t, thread_cp);

  thread->u = *u;
}

void timer_thread_update_timeout(thread_ref_t thread_cp, unsigned long timer) {
  thread_t *thread = no_const(thread_t, thread_cp);
  timeval_t sands;
  ...
  if (timercmp(&thread->sands, &sands, ==)) return;
...
thread_ref_t thread_add_timer_shutdown(
    thread_master_t *m, thread_func_t func, void *arg, unsigned long timer) {
  union {
    thread_t *p;
    const thread_t *cp;
...
/* Add a child thread. */
thread_ref_t thread_add_child(
    thread_master_t *m, thread_func_t func, void *arg, pid_t pid, unsigned long timer) {
  thread_t *thread;
...
void thread_children_reschedule(thread_master_t *m, thread_func_t func, unsigned long timer) {
  thread_t *thread;
...
thread_ref_t thread_add_event(thread_master_t *m, thread_func_t func, void *arg, int val) {
  thread_t *thread;
...
static thread_ref_t thread_add_generic_terminate_event(
    thread_master_t *m, thread_type_t type, thread_func_t func, int status) {
  thread_t *thread;
...
thread_ref_t thread_add_terminate_event(thread_master_t *m) {
  return thread_add_generic_terminate_event(m, THREAD_TERMINATE, NULL, 0);
}

thread_ref_t thread_add_parent_terminate_event(thread_master_t *m, int status) {
  return thread_add_generic_terminate_event(m, THREAD_TERMINATE, NULL, status);
}

thread_ref_t thread_add_start_terminate_event(thread_master_t *m, thread_func_t func) {
  return thread_add_generic_terminate_event(m, THREAD_TERMINATE_START, func, 0);
}

#ifdef USE_SIGNAL_THREADS
/* Add signal thread. */
thread_ref_t thread_add_signal(thread_master_t *m, thread_func_t func, void *arg, int signum) {
  thread_t *thread;
  ...
  if (sigismember(&m->signal_mask, signum)) return thread;
...
void thread_cancel(thread_ref_t thread_cp) {
  thread_t *thread = no_const(thread_t, thread_cp);
  thread_master_t *m;

  if (!thread || thread->type == THREAD_UNUSED) return;
  ...
  switch (thread->type) {
    ...
    case THREAD_READ_ERROR:
      if (thread->event) thread_event_del(thread, THREAD_FL_EPOLL_READ_BIT);
      list_del_init(&thread->e_list);
      break;
    case THREAD_READY_WRITE_FD:
    case THREAD_WRITE_TIMEOUT:
    case THREAD_WRITE_ERROR:
      if (thread->event) thread_event_del(thread, THREAD_FL_EPOLL_WRITE_BIT);
      list_del_init(&thread->e_list);
      break;
    case THREAD_EVENT:
    case THREAD_READY:
    ...
    case THREAD_TERMINATE:
      log_message(
          LOG_WARNING, "ERROR - thread_cancel called for THREAD_%s",
          thread->type == THREAD_TIMER_SHUTDOWN ? "TIMER_SHUTDOWN"
          : thread->type == THREAD_TERMINATE    ? "TERMINATE"
                                                : "TERMINATE_START");
      return;
    default:
      log_message(
          LOG_WARNING, "ERROR - thread_cancel called for unknown thread type %u", thread->type);
      break;
  }
...
void thread_cancel_read(thread_master_t *m, int fd) {
  thread_t *thread, *thread_tmp;
...
#ifdef _WITH_SNMP_
static void snmp_read_thread(thread_ref_t thread) {
  fd_set snmp_fdset;
  thread_event_t *event;
  ...
  if (thread->type == THREAD_READ_ERROR)
    ...
    if (FD_ISSET(thread->u.f.fd, &master->snmp_fdset)) {
      ...
      if (!event || !event->read)
        thread_add_read(
            thread->master, snmp_read_thread, thread->arg, thread->u.f.fd, TIMER_NEVER, 0);
    }
  }
}

void snmp_timeout_thread(thread_ref_t thread) {
  snmp_timeout();
  run_alarms();
  netsnmp_check_outstanding_agent_requests();

  thread->master->snmp_timer_thread =
      thread_add_timer(thread->master, snmp_timeout_thread, thread->arg, TIMER_NEVER);

  snmp_epoll_info(thread->master);
}

// See https://vincent.bernat.im/en/blog/2012-snmp-event-loop
void snmp_epoll_info(thread_master_t *m) {
  fd_set snmp_fdset;
  int fdsetsize = 0;
  int max_fdsetsize;
  int set_words;
...
#error "snmp_epoll_info sizeof(fd_mask) does not match old_set/new_set/diff"
  ...
  if (snmpblock) snmp_timer_wait.tv_sec = TIMER_DISABLED;
  ...
  if (!max_fdsetsize) return;
  set_words = (max_fdsetsize - 1) / (sizeof(*old_set) * CHAR_BIT) + 1;

  /* coverity[ptr_arith] */
  for (i = 0, old_set = PTR_CAST(unsigned long, &m->snmp_fdset),
      new_set = PTR_CAST(unsigned long, &snmp_fdset);
       i < set_words; i++, old_set++, new_set++) {
    if (*old_set == *new_set) continue;
...
static void snmp_epoll_update(thread_master_t *m, bool reset) {
  fd_set snmp_fdset;
  int fdsetsize = 0;
  unsigned set_words;
  struct timeval snmp_timer_wait = {.tv_sec = TIMER_DISABLED};
...
static void snmp_epoll_reset(thread_master_t *m) { snmp_epoll_update(m, true); }

void snmp_epoll_clear(thread_master_t *m) {
...
static list_head_t *thread_fetch_next_queue(thread_master_t *m) {
  ...
  if (!list_empty(&m->event)) return &m->event;

  /* If there are ready threads process them */
  if (!list_empty(&m->ready)) return &m->ready;
  ...
  do {
...
#ifdef _VRRP_FD_DEBUG_
    if (extra_threads_debug) extra_threads_debug();
#endif

#ifdef _EPOLL_THREAD_DUMP_
    if (do_epoll_thread_dump) dump_thread_data(m, NULL);
#endif

#ifdef _EPOLL_DEBUG_
    if (do_epoll_debug) log_message(LOG_INFO, "calling epoll_wait");
#endif
    ...
    if (ret < 0) {
      ...
      if (errno != last_epoll_errno) {
        ...
        log_message(
            LOG_INFO,
            "scheduler: epoll_wait has returned errno %d for 5 successive calls - terminating",
            last_epoll_errno);
    ...
    if (min_auto_priority_delay && timerisset(&earliest_timer)) {
      ...
      if (earliest_timer.tv_sec < 0) {
        ...
        if (timeout > min_auto_priority_delay) {
          log_message(
              LOG_INFO, "A thread timer expired %u.%6.6u seconds ago", timeout / 1000000,
              timeout % 1000000);
        ...
        if (thread_timeout_handler && timeout >= thread_timeout_min)
    ...
    for (i = 0; i < ret; i++) {
      ...
      if (ep_ev->events & (EPOLLHUP | EPOLLERR | EPOLLRDHUP)) {
        ...
        if (__test_bit(LOG_DETAIL_BIT, &debug) && ep_ev->events & EPOLLRDHUP)
      ...
      if (ep_ev->events & EPOLLIN) {
        if (!ev->read) {
          log_message(
              LOG_INFO, "scheduler: No read thread bound on fd:%d (fl:0x%.4X)", ev->fd,
              ep_ev->events);
          continue;
        }
      ...
      if (ep_ev->events & EPOLLOUT) {
        if (!ev->write) {
          log_message(
              LOG_INFO, "scheduler: No write thread bound on fd:%d (fl:0x%.4X)", ev->fd,
              ep_ev->events);
          continue;
        }
...
/* Call thread ! */
static inline void thread_call(thread_t *thread) {
#ifdef _EPOLL_DEBUG_
  if (do_epoll_debug)
    log_message(
        LOG_INFO, "Calling thread function %s(), type %s, val/fd/pid %d, status %d id %lu",
        get_function_name(thread->func), get_thread_type_str(thread->type), thread->u.val,
        thread->u.c.status, thread->id);
#endif

  (*thread->func)(thread);
}

int process_threads(thread_master_t *m) {
  thread_t *thread;
  ...
  while ((thread_list = thread_fetch_next_queue(m))) {
    ...
    if (get_keepalived_mem_err_debug() && __test_bit(MEM_ERR_DETECT_BIT, &debug)
#ifdef _WITH_VRRP_
    ...
    if (!(thread = thread_trim_head(thread_list))) continue;
    ...
    if (!shutting_down ||
        ((thread->type == THREAD_READY_READ_FD || thread->type == THREAD_READY_WRITE_FD ||
          thread->type == THREAD_READ_ERROR || thread->type == THREAD_WRITE_ERROR) &&
         (thread->u.f.fd == m->timer_fd || thread->u.f.fd == m->signal_fd
#ifdef _WITH_SNMP_
          || (snmp_running && FD_ISSET(thread->u.f.fd, &m->snmp_fdset))
#endif
              )) ||
        thread->type == THREAD_CHILD || thread->type == THREAD_CHILD_TIMEOUT ||
        thread->type == THREAD_CHILD_TERMINATED || thread->type == THREAD_TIMER_SHUTDOWN ||
        thread->type == THREAD_TERMINATE) {
      exit_code = thread->u.val;

      if (thread->func) thread_call(thread);

      /* If m->current_thread has been cleared, the thread
       * has been freed. This happens during a reload. */
      thread = m->current_thread;

      if (thread_type == THREAD_TERMINATE_START) shutting_down = true;
    } else if (
        thread->type == THREAD_READY_READ_FD || thread->type == THREAD_READY_WRITE_FD ||
        thread->type == THREAD_READ_TIMEOUT || thread->type == THREAD_WRITE_TIMEOUT ||
        thread->type == THREAD_READ_ERROR || thread->type == THREAD_WRITE_ERROR) {
    ...
    if (thread) {
      m->current_event =
          (thread_type == THREAD_READY_READ_FD || thread_type == THREAD_READY_WRITE_FD)
              ? thread->event
              : NULL;
      thread_add_unuse(m, thread);
      m->current_thread = NULL;
    ...
    if (shutting_down && !m->shutdown_timer_running && !m->child.rb_root.rb_node) break;

    /* If daemon hanging event is received stop processing */
    if (thread_type == THREAD_TERMINATE) return exit_code;
  }

  return 0;
}

static void process_child_termination(pid_t pid, int status) {
  thread_master_t *m = master;
  rb_node_t *thread_node;
  thread_t *thread;

  thread_node = rb_find(&pid, &master->child_pid, thread_child_pid_cmp);
  if (thread_node) thread = rb_entry(thread_node, thread_t, rb_data);

#ifdef _EPOLL_DEBUG_
  if (do_epoll_debug)
    log_message(
        LOG_INFO, "Child %d terminated with status 0x%x, thread_id %lu", pid, (unsigned)status,
        thread_node ? thread->id : 0);
#endif
...
#ifdef _WITH_SNMP_
  if (with_snmp) m->snmp_timer_thread = thread_add_timer(m, snmp_timeout_thread, 0, TIMER_NEVER);
#endif
}

/* Our infinite scheduling loop */
int launch_thread_scheduler(thread_master_t *m) {
...
void register_thread_timeout_handler(void (*func)(unsigned), unsigned min_timeout) {
